...
function wpestate_scripts() {
    ...
    wp_enqueue_style('inttelinput',get_template_directory_uri().'/css/intlTelInput.css', array(), '1.0', 'all');
    wp_enqueue_style('wpestate_media',get_template_directory_uri().'/css/my_media.css', array('bootstrap','bootstrap-theme'), '1.0', 'all');
    wp_enqueue_style( 'icon-font', get_template_directory_uri() . '/libs/icofont/icofont.min.css', array(), '', 'screen' );
    ...
    wp_enqueue_script('validate-js', get_template_directory_uri().'/js/jquery.validate.min.js',array(), '1.0', true);
    // wp_enqueue_script('slick', get_template_directory_uri() . '/js/slick/slick.min.js', array('jquery'), null, true);
    wp_enqueue_script( 'top_slider', get_template_directory_uri().'/js/top_slider.js', array('jquery'), '1.0', true );
    ...
    wp_enqueue_script('select2', get_template_directory_uri().'/js/select2.min.js',array('jquery','moment'), '1.0', true);
    ...
    wp_enqueue_script('intlTelInput.min', get_template_directory_uri().'/js/intlTelInput.min.js',array('jquery'), '1.0', true);

    wp_enqueue_script('ano-custom-js', get_template_directory_uri().'/js/custom.js',array('jquery'), '1.0', true);
    ...
    wp_enqueue_style('custom-style', trailingslashit( get_template_directory_uri() ) . 'css/custom.css');
...
if ( $page_template=='user_dashboard_edit_listing.php' || 
    ...
    $booking_array                  =   array();
    $booking_room_array             =   array();
    $booking_room_available         =   '';
    $property_notbooking            =   array();
    $custom_price                   =   '';
    $default_price                  =   '';
    ...
    $check_booking_plan = array();
    if(isset($post->ID)){
        $custom_price    =  json_encode(  wpml_custom_price_adjust($post->ID));

        $booking_arrays = get_post_meta($post->ID, 'booking_dates',true  );
        if (is_array($booking_arrays)) {
            $booking_array = json_encode(get_post_meta($post->ID, 'booking_dates',true  ));
        } else {
            if (!empty($booking_arrays)) {
                $booking_array = json_encode(unserialize(get_post_meta($post->ID, 'booking_dates',true)));
            } else {
                $booking_array = json_encode($booking_array);
            }
        }

        $booking_room_arrays = get_post_meta($post->ID, 'booking_plans_room',true);

        if (is_array($booking_room_arrays)) {
            $booking_room_array = json_encode(get_post_meta($post->ID, 'booking_plans_room',true));
        } else {
            if (!empty($booking_room_arrays)) {
                $booking_room_array = json_encode(unserialize(get_post_meta($post->ID, 'booking_plans_room',true)));
            } else {
                $booking_room_array = json_encode($booking_room_array);
            }
        }
        ...
        $booking_room_available         =   floatval ( get_post_meta($post->ID, 'property_empty', true));

        $property_notbooking         =   json_encode(get_post_meta($post->ID, 'property_notbooking', true));

        $check_booking_plan = json_encode(wpestate_get_booking_plans_id($post->ID));

        // 2023/2/28 デイユース開発 高橋（追加）---
        $wpestate_currency              =   esc_html   ( get_post_meta($post->ID, 'local_currency', true) );
    ...
    wp_localize_script('wpestate_control', 'control_vars',
            array(  'searchtext'            =>   esc_html__( 'SEARCH','wprentals'),
                    ...
                    'noroomsavailable'                  =>   esc_html__( 'No more rooms available','wprentals'),
                    ...
                    // 2023/7/5 デイユース開発 高橋（追加）---
                    'booking_complete_url'               =>   wpestate_get_template_link('booking_complete.php'),
    ...
    wp_localize_script('wpestate_control', 'control_vars_property',array(
        ...
        'booking_room_array'         =>  $booking_room_array,
        'booking_room_available'         =>  $booking_room_available,
        'property_notbooking'         =>  $property_notbooking,
        ...
        'check_booking_plan'                =>   $check_booking_plan,
    ));
    ...
    if( ( isset($post->ID) && wpestate_check_if_admin_page($post->ID) )  ||  ( 'estate_property' == get_post_type() )  ||  ( 'estate_agent' == get_post_type() )  ){
        ...
        wp_localize_script('wpestate_ajaxcalls_add', 'ajaxcalls_add_vars',
            array(  'admin_url'                 =>  get_admin_url(),
                    ...
                    // 2023/4/30 デイユース開発 高橋（追加）
                    'err_msg_value'                  =>  esc_html__('Value fields of "Plans" must be entered numerically','wprentals'),
                    'err_msg_value_check_in_out'     =>  esc_html__('Check-in time must be set before the Check-out time','wprentals'),
            )
        );
    ...
    if ( is_user_logged_in() ) {
        ...
        wp_localize_script('wpestate_property', 'property_vars',
            array(  'plsfill'                 =>    esc_html__( 'Please fill all the forms!','wprentals'),
                    ...
                    'noplan'                  =>    esc_html__('Please select a plan','wprentals'),
        ...
        //if( wpestate_check_if_admin_page($post->ID) || is_singular('estate_property') ){
                ...
                wp_localize_script('wpestate_dashboard-control', 'dashboard_vars',
                    array(  'deleting'                  =>  esc_html__( 'deleting...','wprentals'),
                            ...
                            'complete_mess'             =>  esc_html__('Are you sure you want to complete?','wprentals'),
                            'completed'                 =>  esc_html__('Completed','wprentals'),
                            ...
                            'select_checkin'            =>  esc_html__( 'check-in time','wprentals'),
                            'select_checkout'            =>  esc_html__( 'check-out time','wprentals'),
                            // 'time_option_0'                         =>  esc_html__( '0:00','wprentals'),
                            // 'time_option_1'                         =>  esc_html__( '1:00','wprentals'),
                            // 'time_option_2'                         =>  esc_html__( '2:00','wprentals'),
                            // 'time_option_3'                         =>  esc_html__( '3:00','wprentals'),
                            // 'time_option_4'                         =>  esc_html__( '4:00','wprentals'),
                            // 'time_option_5'                         =>  esc_html__( '5:00','wprentals'),
                            'time_option_6'                         =>  esc_html__( '6:00','wprentals'),
                            'time_option_7'                         =>  esc_html__( '7:00','wprentals'),
                            'time_option_8'                         =>  esc_html__( '8:00','wprentals'),
                            'time_option_9'                         =>  esc_html__( '9:00','wprentals'),
                            'time_option_10'                         =>  esc_html__( '10:00','wprentals'),
                            'time_option_11'                         =>  esc_html__( '11:00','wprentals'),
                            'time_option_12'                         =>  esc_html__( '12:00','wprentals'),
                            'time_option_13'                         =>  esc_html__( '13:00','wprentals'),
                            'time_option_14'                         =>  esc_html__( '14:00','wprentals'),
                            'time_option_15'                         =>  esc_html__( '15:00','wprentals'),
                            'time_option_16'                         =>  esc_html__( '16:00','wprentals'),
                            'time_option_17'                         =>  esc_html__( '17:00','wprentals'),
                            'time_option_18'                         =>  esc_html__( '18:00','wprentals'),
                            'time_option_19'                         =>  esc_html__( '19:00','wprentals'),
                            'time_option_20'                         =>  esc_html__( '20:00','wprentals'),
                            'time_option_21'                         =>  esc_html__( '21:00','wprentals'),
                            'time_option_22'                         =>  esc_html__( '22:00','wprentals'),
                            'time_option_23'                         =>  esc_html__( '23:00','wprentals'),
                            'time_option_24'                         =>  esc_html__( '24:00','wprentals'),
                    )
                );
...
function wpestate_admin($hook_suffix) {
    ...
    if (is_admin() &&  ( $pagenow=='post-new.php' || $pagenow=='post.php') && $typenow=='estate_property') {
        ...
        if( intval( wprentals_get_option('wp_estate_kind_of_places') ) == 1 ){
            ...
            // $libraries ='&libraries=places&amp;language=ja';
        }
