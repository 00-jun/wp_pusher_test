...
if( !function_exists('wpestate_ajax_add_allinone_custom') ):
    function wpestate_ajax_add_allinone_custom(){
        ...
        echo wpestate_show_price_custom($property_id,$new_custom_price);
...
if( !function_exists('wpestate_ajax_front_end_submit') ):
    function wpestate_ajax_front_end_submit(){
        ...
        if($has_errors){
            ...
            $trigid = wpml_get_content_trid('post_estate_property', $post_id); // Find Transalation ID function from WPML API. 

            //Repeating for JP post.
            $_POST['icl_post_language'] = $language_code = 'ja'; 
            $tpropertyid2 = wp_insert_post( $post );
            $sitepress->set_element_language_details($tpropertyid2, 'post_estate_property', $trigid, $language_code);
        ...
        if($post_id) {
            ...
            $prop_category_selected = array();
            $prop_category = get_terms( 'property_category', array('include' => $_POST['prop_category']) );

            $catsPro = [];
            if(is_array($_POST['prop_category'])) {
                foreach ($_POST['prop_category'] as $key => $value) {
                    $catsPro[] = (int)$value;
                }
                // $prop_category                  =   get_term( $prop_category, 'property_category');
                foreach ($catsPro as $key => $value) {
                    $prop_category_selected[] = (int)$value;
                }
            }
            ...
            wp_set_post_terms( $post_id, $catsPro, 'property_category' );
...
if( !function_exists('wpestate_ajax_add_booking') ):
    function wpestate_ajax_add_booking(){
        ...
        $guest_detail ='';
        if($booking_adults!='' && wprentals_get_option('wp_estate_custom_guest_control','') =='yes' ){
            $guest_detail.='(';
            $guest_detail.= sprintf( _n( 'Adult: %s', 'Adults: %s', $booking_adults, 'wprentals' ), number_format_i18n( $booking_adults ) );
            $guest_detail.= ', '.sprintf( _n( 'Child: %s', 'Children: %s', $booking_childs, 'wprentals' ), number_format_i18n( $booking_childs ) );
            $guest_detail.= ', '.sprintf( _n( 'Infant: %s', 'Infants: %s', $booking_infants, 'wprentals' ), number_format_i18n( $booking_infants ) );
            $guest_detail.=')';
        }
        ...
        // update_post_meta($post_id, 'booking_plans', $plan);
        ...
        $wpestate_currency = esc_html( get_post_meta($property_id, 'local_currency', true) );
        ...
        <div class="create_invoice_form">
            ...
            <div class="invoice_table">
                <div class="invoice_data">
                    ...
                    <span class="date_interval invoice_date_period_wrapper "><span class="invoice_data_legend">'.esc_html__( 'Period','wprentals').' : </span>'.wpestate_convert_dateformat_reverse($fromdate).'</span>
                    
                    <span class="date_duration invoice_date_guest_wrapper"><span class="invoice_data_legend">'.esc_html__( 'No of guests','wprentals').': </span>'.$booking_guest_no.$guest_detail.'</span>';
                ...
                <div class="invoice_details">
                    <div class="invoice_row header_legend">
                        <span class="inv_legend">    '.esc_html__( 'Description','wprentals').'</span>
                        ...
                        foreach ($extra_options_array as $key=>$value){
                            if(isset($extra_pay_options[$value][0])){
                                $extra_option_value                 =   wpestate_calculate_extra_options_value($booking_array['count_days'],$booking_guest_no,$extra_pay_options[$value][2],$extra_pay_options[$value][1]);
                                ...
                                print'
                                <div class="invoice_row invoice_content">
                                    <span class="inv_legend">   '.$extra_pay_options[$value][0].' '.$checkin_out_time[$extra_pay_options[$value][3]].' - '.$checkin_out_time[$extra_pay_options[$value][4]].'</span>
                        ...
                        print'<div class="invoice_row invoice_content">
                                <span class="inv_legend">  ' .esc_html__('Tax','wprentals'). '</span>
                                <span class="inv_data">  ' . $taxes_show . '</span>
                    ...
                    print'
                        ...
                        $label  =   esc_html__( 'Submit Booking Request','wprentals');


                    print '<span id="booking_request_submit" data-propertyid="'.esc_attr($property_id).'" data-bookid="'.esc_attr($bookid).'">'.$label.'</span>';
                    $ajax_nonce = wp_create_nonce( "wprentals_booking_request_submit_nonce" );
                    print'<input type="hidden" id="wprentals_booking_request_submit" value="'.esc_html($ajax_nonce).'" />    ';
      ...
      //         $property_emptys = floatval ( get_post_meta($property_id, 'property_empty', true));
//         $bookked_rooms = floatval ( get_post_meta($property_id, 'booked_rooms', true));

//         $available_room = $property_emptys - $bookked_rooms;

//         if ($status == 'pending') {
//             update_post_meta($post_id, 'booking_list_empty', $available_room);
//         } else{
//             $bookked_rooms=$bookked_rooms+1;
//             update_post_meta($property_id, 'booked_rooms', floatval ($bookked_rooms));
//             update_post_meta($post_id, 'booking_list_empty', floatval ( $property_emptys - $bookked_rooms));
//         }
...
//         $reservation_plan_array = wpestate_get_booking_plans_room($property_id);
//         update_post_meta($property_id, 'booking_plans_room', $reservation_plan_array);
...
if( !function_exists('wpestate_ajax_add_booking_instant') ):
    function wpestate_ajax_add_booking_instant(){
        ...
        // 2023/2/29 デイユース開発 高橋（追加）---
        update_post_meta($booking_id, 'local_currency', $wpestate_currency);
        ...
        //2023/2/12 デイユース開発 高橋（修正）
        update_post_meta($booking_id, 'booking_plans', $plan);
        ...
        if ($status == 'pending') {
            update_post_meta($booking_id, 'booking_list_empty', $available_room);
        } else{
            $bookked_rooms=$bookked_rooms+1;
            update_post_meta($property_id, 'booked_rooms', floatval ($bookked_rooms));
            update_post_meta($booking_id, 'booking_list_empty', floatval ( $property_emptys - $bookked_rooms));
        }
        ...
        // Re build the plans array
        $reservation_plan_array = wpestate_get_booking_plans_room($property_id);
        if($reservation_plan_array==''){
            $reservation_plan_array = wpestate_get_booking_dates($property_id);
        }
        update_post_meta($property_id, 'booking_dates', $reservation_array);
        ...
        if( is_array($extra_options_array) &&  !empty ( $extra_options_array )  ){
            ...
            foreach ($extra_options_array as $key=>$value){
                if( isset($extra_pay_options[$value][0]) ){
                    ...
                    $temp_array         =   array($extra_pay_options[$value][0].' '.$checkin_out_time[$extra_pay_options[$value][3]].' - '.$checkin_out_time[$extra_pay_options[$value][4]],$value_computed,$extra_option_value_show_single);
                    $details[]          =   $temp_array;
                }
            }
        }
        ...
        // 2023/2/29 デイユース開発 高橋（追加）---
        update_post_meta($invoice_id, 'local_currency', $wpestate_currency);
       ...
       // $depozit            =   round($wp_estate_book_down*$total_price_comp/100,2);
        ...
        <div class="create_invoice_form">
                ...
                <div class="invoice_table">
                    <div class="invoice_data">
                            <span class="date_interval invoice_date_period_wrapper"><span class="invoice_data_legend">'.esc_html__( 'Period ','wprentals').' : </span>'.wpestate_convert_dateformat_reverse($booking_from_date).'</span>
                            <span class="date_duration invoice_date_guests_wrapper"><span class="invoice_data_legend">'.esc_html__( 'No of guests','wprentals').': </span>'.$booking_guests.wpestate_booking_guest_explanations($booking_id).'</span>';
                          
                                // 2023/2/3 デイユース開発 高橋（コメントアウト）---
                            ...
                            foreach ($extra_options_array as $key=>$value){
                                if( isset($extra_pay_options[$value][0]) ){
                                    ...
                                    <div class="invoice_row invoice_content">
                                        ...
                                        <span class="inv_data">'.$extra_option_value_show_single.'</span>
                                    </div>';
                                }
                            }
        ...
        if( isset( $_POST['listing_edit'] ) ) {
            if( !is_numeric($_POST['listing_edit'] ) ){
                ...
                if( $current_user->ID != $the_post->post_author ) {
                    ...
                    $is_publishing                  =   intval($_POST['is_publishing']);

                    // 2023/6/27 デイユース開発 高橋（追加）---
                    update_post_meta($edit_id, 'is_publishing',$is_publishing);
                    ...
                    // 2023/6/27 デイユース開発 高橋（修正）---
                     if($status=='pending' || $status=='pending_no_publishing'){
                        if($status=='pending'){
                            $message_status=esc_html__( 'Your listing is pending. Please complete all the mandatory fields for it to be published!','wprentals');
                        }else{
                            $message_status=esc_html__( 'Your listing is pending. Please check the PUBLISH button for it to be published!','wprentals');
                        }
                        echo json_encode(array('edited'=>true, 'response'=>esc_html__( 'Changes are saved !','wprentals').' '.$message_status));                        
                    }else{
                        echo json_encode(array('edited'=>true, 'response'=>esc_html__( 'Now published !','wprentals')));
                    }
        ...
        if( isset( $_POST['listing_edit'] ) ) {
            if( !is_numeric($_POST['listing_edit'] ) ){
                ...
                if( $current_user->ID != $the_post->post_author ) {
                    ...
                    $array_notbooking = $_POST['array_notbooking'];

                    update_post_meta($edit_id, 'property_notbooking', $array_notbooking);
                    ...
                    $id_translations = get_id_translations($edit_id, apply_filters( 'wpml_current_language', NULL ));

                    update_post_meta($id_translations, 'property_notbooking', $array_notbooking);
                    
                    // 2023/3/12 デイユース開発 高橋（追加）
                    $status         =   wpestate_global_check_mandatory($edit_id);
                    $message_status =   '';
        ...
        if( isset( $_POST['listing_edit'] ) ) {
            if( !is_numeric($_POST['listing_edit'] ) ){
                ...
                if( $current_user->ID != $the_post->post_author ) {
                    ...
                    $allowed_html           =   array();
                    $property_empty          =   floatval($_POST['property_empty']);
                    ...
                    //$booked_rooms = floatval((int)(0));

                    //update_post_meta($edit_id, 'booked_rooms', $booked_rooms);
                    $room_empty = floatval(get_post_meta( $edit_id, 'property_empty', true));
                    if ($property_empty) {
                        update_post_meta($edit_id, 'property_empty', $property_empty);
                    } else {
                        update_post_meta($edit_id, 'property_empty', floatval(0));
                    }
                    ...
                    if (is_array($room_id) && !empty($room_id)) {
                        update_post_meta($edit_id, 'room_ids', $room_id);
                    } else {
                        update_post_meta($edit_id, 'room_ids', []);
                    }
                    ...
                    //     $message_status=esc_html__( 'Your listing is pending. Please complete all the mandatory fields for it to be published!','wprentals');
                    // }
                    // echo json_encode(array('edited'=>true,'room_id'=>$room_id,'room_empty'=>$room_empty,'beds_options'=>count($beds_options),'response'=>esc_html__( 'Changes are saved!','wprentals').' '.$message_status));
            ...
            if( !is_numeric($_POST['listing_edit'] ) ){
                ...
                if( $current_user->ID != $the_post->post_author ) {
                    ...
                    if (empty($attachid)) {
                        ...
                        echo json_encode(array('edited'=>false,'attach_array'=>$_POST['attachid'] , 'response'=>esc_html__( 'Error: do not leave the image blank','wprentals')));
                        exit();
                    }
                    ...
                    // if($video_id == '' || $attachid){
                    //     echo json_encode(array('edited'=>true, 'response'=>esc_html__( 'Changes are saved test-2!','wprentals').' '.$message_status));
                    // }
                    ...
                    //     $message_status=esc_html__( 'Your listing is pending. Please complete all the mandatory fields for it to be published!','wprentals');
                    // }
                    // echo json_encode(array('edited'=>true,'attach_array'=>$attach_array , 'response'=>esc_html__( 'Changes are saved !','wprentals').' '.$message_status));
        ...
        if( isset( $_POST['listing_edit'] ) ) {
            if( !is_numeric($_POST['listing_edit'] ) ){
                ...
                if( $current_user->ID != $the_post->post_author ) {
                    ...
                    $local_currency                 =   esc_html( $_POST['local_currency']);
                    ...
                    // 2023/2/27 デイユース開発 高橋（追加）---
                    update_post_meta($edit_id, 'local_currency', $local_currency);
    ...
    if( isset( $_POST['listing_edit'] ) ) {
        if( !is_numeric($_POST['listing_edit'] ) ){
            ...
            ////////////////////////////////////////////////////////////////////
                ...
                $prop_category_selected = array();
                $prop_category = get_terms( 'property_category', array('include' => $_POST['category']) );
                ...
                // if($submit_title == ''){
                //     $has_errors = true;
                //     $errors['title'] = esc_html__( 'Please complete these fields: Hotel name','wprentals');
                // }

                // if( is_array($mandatory_page_fields) && in_array('prop_category', $mandatory_page_fields)) {
                //     if($prop_category=='' || $prop_category=='-1'){
                //         $has_errors=true;
                //         $errors['category']=esc_html__( 'Please submit a category for your property','wprentals');
                //     }
                // }
                ...
                if( is_array($mandatory_page_fields) && in_array('prop_action_category_submit', $mandatory_page_fields)) {
                    if($prop_action_category=='' || $prop_action_category=='-1'){
                        $has_errors=true;
                        $errors['second_category'] = esc_html__( 'Please submit a  the second category for your property','wprentals');
                    }
                }
                ...
                //     if($property_city==''){
                //         $has_errors=true;
                //         $errors['city'] = esc_html__( 'Please submit a city for your listing','wprentals');
                //     }
                // }
                ...
                if( is_array($mandatory_page_fields) && in_array('property_affiliate', $mandatory_page_fields)) {
                    if($property_affiliate=='' ){
                        $has_errors=true;
                        $errors['affiliate']=esc_html__( 'Please submit an affiliate link','wprentals');
                    }
                }
                ...
                if( is_array($mandatory_page_fields) && in_array('max_extra_guest_no', $mandatory_page_fields)) {
                    if($max_extra_guest_no=='' ){
                        $has_errors=true;
                        $errors['maximum_extra']=esc_html__( 'Please submit an maximum extra guests above capacity number','wprentals');
                    }
                }
                ...
                if($has_errors){
                    ...
                    echo json_encode(array('edited'=>false, 'response'=>$show_err, 'ele_err'=>$ele_err));
                }else{
                    $post = array(
                        'ID'            => $edit_id,
                        'post_title'    => $submit_title,
                    ...
                    // 2023/6/27 デイユース開発 高橋（追加）---
                    update_post_meta($post_id, 'is_publishing',$is_publishing);
                    ...
                    // 2023/3/11 デイユース開発 高橋（追加）---
                    update_post_meta($post_id, 'property-name', $submit_title);
                    ...
                    // }else if($status=='pending_no_publishing'){
                    //     $message_status=esc_html__( 'Your listing is pending. Please check the PUBLISH button for it to be published!','wprentals');
                    // }
                    // echo json_encode(array('edited'=>true, 'category'=>$catsPro, 'category_selected'=> $prop_category_selected, 'response'=>esc_html__( 'Changes are saved!','wprentals').' '.$message_status));
                }
...
//wpestate_duplicate_property
add_action('wp_ajax_nopriv_wpestate_duplicate_property', 'wpestate_duplicate_property');
add_action('wp_ajax_wpestate_duplicate_property', 'wpestate_duplicate_property');

if (!function_exists('wpestate_duplicate_property')):
    function wpestate_duplicate_property()
    {
        $current_user   =   wp_get_current_user();
        $userID         =   $current_user->ID;
        if (!is_user_logged_in()) {
        ...
        $args = array(
            ...
            'author'           =>  $userID,
            'posts_per_page'    => -1,
            'post_status' => array( 'pending', 'publish', 'disabled' ),
            'p' => $prop_id
        );

        //$post_id = wp_insert_post( $my_post );

        $prop_selector = new WP_Query($args);

        $newID = '';

        while ($prop_selector->have_posts()): $prop_selector->the_post();
            $current_ID = get_the_ID();
            $prop_cats = wp_get_post_terms( $current_ID, 'property_category',array('fields' => 'ids') );;
            $prop_type = wp_get_post_terms( $current_ID, 'property_action_category',array('fields' => 'ids') );
            $prop_city = wp_get_post_terms( $current_ID, 'property_city',array('fields' => 'ids') );
            $prop_area = wp_get_post_terms( $current_ID, 'property_area',array('fields' => 'ids') );
            $property_features = wp_get_post_terms( $current_ID, 'property_features',array('fields' => 'ids') );

            $guest_no = esc_html(get_post_meta($current_ID, 'guest_no', true) );
            $property_address = esc_html(get_post_meta($current_ID, 'property_address', true) );
            $property_county = esc_html(get_post_meta($current_ID, 'property_county', true) );
            $property_state = esc_html(get_post_meta($current_ID, 'property_state', true) );
            $property_zip = esc_html(get_post_meta($current_ID, 'property_zip', true) );
            $property_country = esc_html(get_post_meta($current_ID, 'property_country', true) );
            $private_notes = esc_html(get_post_meta($current_ID, 'private_notes', true) );
            $instant_booking = esc_html(get_post_meta($current_ID, 'instant_booking', true) );
            $property_latitude = esc_html(get_post_meta($current_ID, 'property_latitude', true) );
            $property_longitude = esc_html(get_post_meta($current_ID, 'property_longitude', true) );
            $google_camera_angle = esc_html(get_post_meta($current_ID, 'google_camera_angle', true) );
            $page_custom_zoom = esc_html(get_post_meta($current_ID, 'page_custom_zoom', true) );

            $extra_pays_options = get_post_meta($current_ID,  'extra_pay_options', true);

            $prop_post = array(
                'post_title'  => get_the_title().' (Copy)',
                'post_content' => get_the_content(),
                'post_status' => 'pending',
                'post_type'   => 'estate_property',
            );
            $propdup_id = wp_insert_post( $prop_post );

            $prop_thumbID = get_post_thumbnail_id($current_ID);
            if ($prop_thumbID) {
                set_post_thumbnail( $propdup_id, $prop_thumbID );
            }

            wp_set_object_terms( $propdup_id, $prop_cats, 'property_category' );
            wp_set_object_terms( $propdup_id, $prop_type, 'property_action_category' );
            wp_set_object_terms( $propdup_id, $prop_city, 'property_city' );
            wp_set_object_terms( $propdup_id, $prop_area, 'property_area' );
            wp_set_object_terms( $propdup_id, $property_features, 'property_features' );

            // update_post_meta( $propdup_id, 'guest_no', $guest_no );
            // update_post_meta( $propdup_id, 'property_address', $property_address );
            // update_post_meta( $propdup_id, 'property_county', $property_county );
            // update_post_meta( $propdup_id, 'property_state', $property_state );
            // update_post_meta( $propdup_id, 'property_zip', $property_zip );
            // update_post_meta( $propdup_id, 'property_country', $property_country );
            // update_post_meta( $propdup_id, 'private_notes', $private_notes );
            // update_post_meta( $propdup_id, 'instant_booking', $instant_booking );
            // update_post_meta( $propdup_id, 'property_latitude', $property_latitude );
            // update_post_meta( $propdup_id, 'property_longitude', $property_longitude );
            // update_post_meta( $propdup_id, 'google_camera_angle', $google_camera_angle );
            // update_post_meta( $propdup_id, 'page_custom_zoom', $page_custom_zoom );

            // duplicate all post meta
            $propdup_meta = get_post_meta( $current_ID );
            if( $propdup_meta ) {
                foreach ( $propdup_meta as $meta_key => $meta_values ) {
                    if( 'booking_plans_room' == $meta_key ) { // do nothing for this meta key
                        continue;
                    }
                    foreach ( $meta_values as $meta_value ) {
                        add_post_meta( $propdup_id, $meta_key, $meta_value );
                    }
                }
            }

            if ($extra_pays_options) {
                update_post_meta( $propdup_id, 'extra_pay_options', $extra_pays_options );
            }

            do_action( 'wpml_make_post_duplicates', $propdup_id );
            // $trigid = wpml_get_content_trid('post_estate_property', $post_id); // Find Transalation ID function from WPML API. 
...
endif; // end   wpestate_duplicate_property