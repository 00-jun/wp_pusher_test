    ...
    function wprentals_get_owner_section($prop_id){
        ...
        if ($agent_id!=0){        
            ...
            if( $agent_selection->have_posts() ){
                   ...
                   while ($agent_selection->have_posts()): $agent_selection->the_post(); 
                        ...
                        // 2023/1/26 デイユース開発 高橋（修正）
                        // $name                = get_the_title();
                        $name                = esc_html( get_user_meta($owner_id, 'hotel_name', true) );
        ...
        <div class="col-md-9 agentpic-wrapper">
            ...
            if($link!=''){
                ...
                $return_string.='<a class="owner_read_more " href="'.esc_url($link).'">'. esc_html__('See accommodation profile','wprentals').'</a>';
...
function wpestate_sleeping_situation_wrapper($post_id, $wp_estate_sleeping_text)
{
    ...
    if (!is_array($beds_options)) {
        ...
        if (!empty($beds_options)) {
            $beds_options = unserialize($beds_options);
        } else {
            return;
        }
    }
    ...
    if ($property_bedrooms!=0) {
        ...
        if ($wp_estate_sleeping_text!='') {
            ...
            $return_string.= esc_html__('Bed type', 'wprentals');
        }
...
if (!function_exists('estate_listing_price')):
    function estate_listing_price($post_id)
    {
        ...
        if (is_array($extra_pay_options) && !empty($extra_pay_options)) {
            $return_string.='<div class="listing_detail list_detail_prop_book_starts_end col-md-12"><span class="item_head">'.esc_html__('Plans', 'wprentals').':</span></div>';
            foreach ($extra_pay_options as $key=>$wpestate_options) {
                ...
                // 2023/3/12 デイユース開発 高橋（修正）
                $return_string.= '<span class = "plan_option_name">'.$wpestate_options[0].'</span><span class = "plan_option_checkin"> '.$checkin_out_time[$wpestate_options[3]].'</span><span class = "plan_option_dash"> - </span><span class = "plan_option_checkout"> '.$checkin_out_time[$wpestate_options[4]].'</span><span class = "plan_option_colon">: </span><span class = "plan_option_value"> '. $extra_option_price_show.'</span>';
...
if (!function_exists('wpestate_show_custom_details')):
    function wpestate_show_custom_details($edit_id, $is_dash=0){
        ...
        // 2023/2/29 デイユース開発 高橋（修正）---
        // $wpestate_currency              =   esc_html(wprentals_get_option('wp_estate_currency_label_main', ''));
        $wpestate_currency              =   esc_html   ( get_post_meta($edit_id, 'local_currency', true) );
...
if (!function_exists('wpestate_sleeping_situation')):
    function wpestate_sleeping_situation($post_id)
    {
        ...
        $beds_options=get_post_meta($post_id, 'property_bedrooms_details', true);
        if (!is_array($beds_options) && $beds_options!='') {
            $beds_options = unserialize($beds_options);
        }
...
if (!function_exists('estate_listing_details')):
    function estate_listing_details($post_id)
    {
        ...
        // }
        if ($property_size != '') {
            $property_size  = number_format($property_size) . ' '.__($measure_sys.'<sup>2</sup>','wprentals');
            if (wprentals_get_option('wp_estate_item_rental_type')!=1) {
        ...
        if (!empty($custom_details)) {
            if (!is_array($custom_details)) {
                $custom_details = unserialize($custom_details);
            }
