...
function wpestate_global_check_mandatory($post_id){
    ...
    $is_publishing = intval(get_post_meta($post_id,'is_publishing',true));
    if(0==$is_publishing){
        $new_status='pending';
        $no_publishing = 1;    
    }
        ...
        // 2023/7/23 デイユース開発 高橋（修正） */
        // $id_ja = apply_filters( 'wpml_object_id', $post_id, 'object', false, 'en' );
        // $trans_post = array(
        //     'ID'            => $id_ja,
        //     'post_status'   => $new_status
        // );
        // wp_update_post( $my_post );
        $id_en = apply_filters( 'wpml_object_id', $post_id, 'post', TRUE, 'en' );
        $original_post = array(
            'ID'            => $id_en,
            'post_status'   => $new_status
        );
        $id_ja = apply_filters( 'wpml_object_id', $post_id, 'post', TRUE, 'ja' );
        $trans_post = array(
            'ID'            => $id_ja,
            'post_status'   => $new_status
        );
        ...
        // 2023/6/27 デイユース開発 高橋（追加）---
        if($no_fill_mandatry==1){
            return $new_status;
        }else{
            if($no_publishing == 1){
                return 'pending_no_publishing';
            }else{
                return $new_status;
            }
        }
...
function wpestate_show_price_label_slider($min_price_slider,$max_price_slider,$wpestate_currency,$wpestate_where_currency){
    ...
    if( !empty($custom_fields) && isset($_COOKIE['my_custom_curr']) &&  isset($_COOKIE['my_custom_curr_pos']) &&  isset($_COOKIE['my_custom_curr_symbol']) && $_COOKIE['my_custom_curr_pos']!=-1){
        ...
        if ($custom_fields[$i][3] == 'before') {
            ...
            $price_slider_label =  $min_price_slider.' '.esc_html__( 'to','wprentals').' '. $max_price_slider;      
