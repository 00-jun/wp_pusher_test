...
<div class="col-md-12">
    ...
    <h4 class="user_dashboard_panel_title"><?php  esc_html_e('Non-reservable dates','wprentals');?></h4>
        ...
        if($wprentals_is_per_hour==2 ){
            ...
            <div class="price_explaning"> <?php   esc_html_e('Select the dates you want to make reservations unavailable.','wprentals');?></div>
            ...
            <div class="col-md-12 calendar-actions">
                ...
                <div class="calendar-legend-reserved calendar-owner-reserved"></div><span><?php esc_html_e('Dates Booked','wprentals');?></span>
                <div class="calendar-legend-reserved calendar-owner-notavailable"></div><span><?php esc_html_e('Room not available','wprentals');?></span>
...
$reservation_class  =   '';
    ...
    function    wpestate_draw_month($month_no,$reservation_array, $unixmonth, $daywithpost,$thismonth,$thisyear,$last_day){
            ...
            $listing_edit_id = $_GET['listing_edit'];
            $rarray = wpestate_get_booking_plans_id($listing_edit_id);

            $dhash = array_flip_multiple($rarray);
            // $dhash = array_filter($dhash, function($items) {return count($items) > 1;});

            $week_begins = intval(get_option('start_of_week'));

            $property_notbooking = wpestate_get_not_available_rooms($listing_edit_id);
            ...
            for ( $day = 1; $day <= $daysinmonth; ++$day ) {
                    ...
                    if ( $day == gmdate('j', current_time('timestamp')) && $thismonth == gmdate('m', current_time('timestamp')) && $thisyear == gmdate('Y', current_time('timestamp')) ){
                        ...
                        if(array_key_exists ($timestamp_java,$dhash) ){
                          ...
                          $start_reservation  =   0;$end_reservation    =   1;
                            ...
                            $calendar_output .= '<td class="calendar-reserved '.esc_attr($has_past_class).' '.esc_attr($reservation_class).'"     data-curent-date="'.esc_attr($timestamp_java).'">'. wpestate_draw_reservation_plans($dhash[$timestamp_java]);
                        }else{
                    ...
                    // else if(array_key_exists ($timestamp_java,$reservation_array) ){ // check for reservation
                    else if(array_key_exists ($timestamp_java,$dhash) ){ // check for reservation
                        $end_reservation=1;
                        ...
                        $calendar_output .= '<td class="calendar-reserved '.esc_attr($has_past_class.$reservation_class).' "     data-curent-date="'.esc_attr($timestamp_java).'">'. wpestate_draw_reservation_plans($dhash[$timestamp_java]);
                    }
                        ...
                        $not_availabel = ' calendar_can_available';
                        $notbooking_class = '';
                        $notbooking_selected = '';

                        if(!is_array($property_notbooking)){
                            $all_unserialized =  unserialize($property_notbooking);
                        } else {
                            $all_unserialized =  $property_notbooking;
                        }


                        if (in_array($timestamp_java,$all_unserialized)) {
                            $notbooking_class = ' calendars-not-available';
                            $notbooking_selected = ' not-available-selected';
                        }

                        $calendar_output .= '<td class="calendar-free '.esc_attr($has_past_class.$reservation_class.$not_availabel.$notbooking_class).'"          data-curent-date="'.esc_attr($timestamp_java).'">'.'<span class="'.$not_availabel.$notbooking_selected.'" data-date="'.esc_attr($timestamp_java).'"></span>';
                    }
...
function wpestate_draw_reservation_plans($reservation_plans){
    if ( is_array($reservation_plans) ){
            $listing_edit_id = $_GET['listing_edit'];
            $calendar_bookid_output = '';
        ?>
            <!-- 2023/6/11 デイユース開発 高橋（修正）--- -->
            <?php foreach ($reservation_plans as $key => $value): ?>


                <?php $status = get_post_meta($value, 'booking_status', true)?>
                <?php if(!($status == 'canceled' || $status == 'request_canceled')){ ?>
                    <?php $calendar_bookid_output .= '<div id="booking_ids" class="booking_ids" data-listing-id="'.$listing_edit_id.'" data-booking-confirmed="'.$value.'">'.$value.'</div>'; ?>
                    <?php //$calendar_bookid_output .= '<div id="booking_ids" class="booking_ids" data-listing-id="'.$listing_edit_id.'" data-booking-confirmed="'.$value.'">'.$value.'<span>×</span></div>'; ?>
                <?php } ?>
            <?php endforeach ?>
        <?php
        return '<div class="rentals_reservation" >'.'<h3 class="booking_title_id">'.esc_html__('Booking id','wprentals').': </h3>'.$calendar_bookid_output.'</div>';
    }
}

function wpestate_get_not_available_rooms($edit_id){
    $property_notbooking = get_post_meta($edit_id, 'property_notbooking',true  );
    if (empty($property_notbooking)) {
        $property_notbooking = [];
    }
    return $property_notbooking;
}
