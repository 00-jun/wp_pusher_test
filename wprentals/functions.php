...
////////////////////////////////////////////////////////////////////////////////////////////////
function wpestate_add_bookings_metaboxes_custom() {    
  add_meta_box(  'estate_booking-roomsid', esc_html__(  'Available rooms', 'wprentals-core' ), 'wpestate_booking_meta_function_custom', 'wpestate_booking' ,'normal','default');
}
add_action('add_meta_boxes', 'wpestate_add_bookings_metaboxes_custom');

////////////////////////////////////////////////////////////////////////////////////////////////
// booking details
////////////////////////////////////////////////////////////////////////////////////////////////

function wpestate_booking_meta_function_custom( $post ) {
    wp_nonce_field( basename( __FILE__ ), 'estate_booking_noncename' );
    global $post;
    ...
    <p class="meta-options">
        <label for="booking_listing_name">'.esc_html__( 'Room ID:','wprentals-core').' </label><br />
        <input type="text" id="booking_room_id" name="booking_room_id" value=""> 
    </p>  
    <p class="meta-options">
        <label for="booking_listing_name">'.esc_html__( 'Plan:','wprentals-core').' </label><br />
        <input type="text" id="booking_room_id" name="plan_room" value=""> 
    </p>  
    ';
}
...
//         wp_enqueue_style('style', get_stylesheet_directory_uri() . '/slick/slick-theme.css');
//         wp_enqueue_style('style', get_stylesheet_directory_uri() . '/slick/slick.css');
//     }
...
//     wp_enqueue_script( ‘script’, get_theme_file_uri(‘/slick/slick.min.js’), [‘jquery-js’], ”, true );
//     wp_enqueue_script( ‘script’, get_theme_file_uri(‘/slick/top_hotel_list.js’), [‘jquery-js’], ”, true );
//     }
//     add_action( ‘wp_enqueue_scripts’, ‘javascript’);

//金本追記press_post.cssを読み込み//


add_action( 'wp_ajax_anothemes_create_booking', 'anothemes_create_booking' );
add_action( 'wp_ajax_nopriv_anothemes_create_booking', 'anothemes_create_booking' );
function anothemes_create_booking() {
    $plan[] = $_POST['plan']; 
    ...
    $booking_guests = (int)$infants + (int)$childs + (int)$adults;

    // Create post object
    $args = array(
      'post_title'    => wp_strip_all_tags( 'Booking Request' ),
    ...
    if(!is_wp_error($post_id)){
      $my_post = array(
          'ID'           => $post_id,
          'post_title'   => 'Booking Request ' . $post_id,
      );
      ...
      update_post_meta( $post_id, 'booking_from_date', $sdate );
      update_post_meta( $post_id, 'booking_to_date', $edate );
      update_post_meta( $post_id, 'booking_guests', $booking_guests );
      update_post_meta( $post_id, 'booking_adults', $adults );
      update_post_meta( $post_id, 'booking_childs', $childs );
      update_post_meta( $post_id, 'booking_infants', $infants );
      update_post_meta( $post_id, 'booking_infants', $infants );
      update_post_meta( $post_id, 'booking_list_empty', 1 );
      update_post_meta( $post_id, 'booking_plans', $plan );

      update_post_meta( $post_id, 'booking_fullname', $fullname );
      update_post_meta( $post_id, 'booking_mail', $mail );
      update_post_meta( $post_id, 'booking_tel', $tel );
      wp_send_json_success('ok');
    }else{
      
      wp_send_json_success($post_id->get_error_message());
    }
...
function custom_post_type() {
    register_post_type( 'whats_new', [ //  $post_type（投稿タイプ名）を指定
        'labels' => [
            'name'          => "What's New", // 管理画面のメニューで表示される投稿タイプの名前
            'singular_name' => 'whats_new',    // この投稿タイプのオブジェクト 1 個の名前（単数形）。デフォルトは 'name' の値。
        ],
    ...
    register_post_type( 'slider', [ //  $post_type（投稿タイプ名）を指定
        'labels' => [
            'name'          => "スライダー", // 管理画面のメニューで表示される投稿タイプの名前
            'singular_name' => 'slider',    // この投稿タイプのオブジェクト 1 個の名前（単数形）。デフォルトは 'name' の値。
        ],
        'public'        => true,  // 投稿タイプをパブリックにするかどうか。true の場合、管理画面とフロントエンド（ユーザー）の両方から利用可能。
        'has_archive'   => true, // この投稿タイプのアーカイブを有効にする。アーカイブのスラッグとして $post_type が使われる。
        'menu_position' => 5,     // この投稿タイプが表示されるメニューの位置。「5」は「投稿」の下。
    ...
    register_post_type( 'press_new', [ //  $post_type（投稿タイプ名）を指定
        'labels' => [
            'name'          => "Press New", // 管理画面のメニューで表示される投稿タイプの名前
            'singular_name' => 'press_new',    // この投稿タイプのオブジェクト 1 個の名前（単数形）。デフォルトは 'name' の値。
        ],
        'public'        => true,  // 投稿タイプをパブリックにするかどうか。true の場合、管理画面とフロントエンド（ユーザー）の両方から利用可能。
        'has_archive'   => true, // この投稿タイプのアーカイブを有効にする。アーカイブのスラッグとして $post_type が使われる。
        'menu_position' => 5,     // この投稿タイプが表示されるメニューの位置。「5」は「投稿」の下。
...
function get_thumbnail_from_post($post_id) {
    $thumbnail_img = get_the_post_thumbnail_url($post_id);
    return $thumbnail_img ? $thumbnail_img : '画像'; // アイキャッチ画像のURLを取得
}
...
function custom_enqueue_styles() {
    wp_enqueue_style( 'press-style', get_template_directory_uri() . '/press_style.css', array(), '1.0.0' );
}
