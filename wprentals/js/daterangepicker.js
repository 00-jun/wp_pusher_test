    ...
    DateRangePicker.prototype = {
                   ...
                   //|| this.element[0].id==='end_date'
                    ...
                    //|| this.element[0].id==='end_date') 
                        if (typeof wpestate_show_price_Daterangepicker === "function" && this.element[0].id==='start_date'  ){ 
                            var received_date       =   new Date(infome_unix);
                            ...
                            property_notbooking  =   control_vars_property.property_notbooking;
                            var room_counts = {};
                            ...
                            // jQuery.each(booking_room_array, function(key,value) {
                            ...
                            property_notbookings = [];
                            if (property_notbooking !== '' && property_notbooking.length !== 0) {
                                property_notbookings = JSON.parse(property_notbooking);
                            }

                            if(check_property_notbooking(property_notbookings, unixtime1_key)) {
                                cname = cname.replace('available', '');
                            }

                            var roomempty = (booking_room_available - room_counts[unixtime1_key]) ? (booking_room_available - room_counts[unixtime1_key]) : '';

                            html += '<td class="infome_unix wpestate_booking_class ' + cname.replace(/^\s+|\s+$/g, '') + '" data-custom="'+infome2+'" data-roomempty="'+roomempty+'" data-title="' + 'r' + row + 'c' + col + '">' + calendar[row][col].date() + '<div class="wpestate_show_available_calendar">'+(roomempty ? ('<span>'+roomempty+' rooms</span>') : '')+'</div><div class="wpestate_show_price_calendar">'+wpestate_show_price_Daterangepicker(infome_unix)+'</div></td>';
                        }else{
        ...
        clickDate: function(e) {
            ...
            if (this.endDate || date.isBefore(this.startDate, 'day')) { //picking start
                ...
                // var fd = moment($(e.target).attr("data-custom")).format("DD/MM/YYYY");
                var input_val = input_val.replace(input_val.substr(0, 2), currYear);
                input_val = input_val.replace(/\-/g, '/');

                // console(input_val.format(date_format));
