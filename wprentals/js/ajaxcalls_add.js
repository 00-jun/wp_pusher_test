...
jQuery(document).ready(function ($) {
  ...
  $('#form_submit_1').on('click', function () {
    ...
    prop_category = prop_category.map(Number);
  ...
  $('#owner_reservation_modal').click(function (e) {
    if (e.target == $(this)[0]) {
      $('#close_reservation_internal').trigger('click');
    }
  });
  ...
  $('.booking-calendar-wrapper-in .has_future').on('click', function (event) {
    if (event.target == $(this)[0]) {
      if (!$(this).hasClass('calendars-not-available')) {
        $('.calendar-reserved').not(this).find('.rentals_reservation').hide();
        if ($(this).hasClass('calendar-reserved')) {
          // click on a booked spot

          $(this).find('.rentals_reservation').show();
  ...
  // custom adding not available
  $('span.calendar_can_available').on('click', function (event) {
    if ($(this).hasClass('not-available-selected')) {
      $(this).removeClass('not-available-selected');
      $(this).parent().removeClass('calendars-not-available');
      $(this).parent().removeClass('calendar_can_available');
    } else {
      $(this).addClass('not-available-selected');
      $(this).parent().addClass('calendars-not-available');
    }
  });
  ...
  $('#edit_prop_ammenities').on('click', function () {
    var jsonData, ajaxurl, listing_edit, custom_fields_amm, counter, i, is_publishing;
    listing_edit = jQuery('#listing_edit').val();
    ...
    is_publishing = 0;
    if (jQuery('#is_publishing').is(':checked')) {
      is_publishing = 1;
    }
  ...
  $('#edit_prop_locations').on('click', function () {
    var jsonData, ajaxurl, listing_edit, property_county, property_state, property_address, property_zip, property_latitude, property_longitude, google_camera_angle, property_google_view, is_publishing;
  ...
  $('#edit_calendar').on('click', function () {
    var jsonData, ajaxurl, listing_edit, property_icalendar_import, array_labels, array_feeds, is_publishing;
    ...
    var array_notbooking = [];
    $('.not-available-selected').each(function () {
      array_notbooking.push($(this).attr('data-date'));
    });
    ...
    $.ajax({
      ...
      data: {
        ...
        array_notbooking: array_notbooking,
        // 2023/6/27 デイユース開発 高橋（修正）---
        is_publishing: is_publishing,
        security: nonce,
      },
  ...
  $('#edit_prop_details').on('click', function () {
    'use strict';
    var value, temp_opt, i, jsonData, ajaxurl, property_empty, property_size, property_rooms, property_bedrooms, property_bathrooms, listing_edit, custom_fields_val, variable, counter, is_publishing;
    property_empty = jQuery('#property_empty').val();
    ...
    var cancellation_policy = jQuery('#cancellation_policy').val().trim();
    var other_rules = jQuery('#other_rules').val().trim();
    ...
    var room_id = [];
    var roomIds = $('#property_empty').attr('data-oldroom');
    room_id = roomIds.split(',');
  ...
  $('#edit_prop_image').on('click', function () {
    var ajaxurl, video_type, video_id, attachid, attachthumb, listing_edit, virtual_tour, is_publishing;
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.edited) {
          ...
          $('#profile_message')
            .empty()
            .append('<div class="login-alert field_err">' + data.response + '<div>');
        }
        var redirect = jQuery('.next_submit_page').attr('href');
        window.location = redirect;
      },
  ...
  $('#edit_prop_price').on('click', function () {
    var booking_start_hour,
      ...
      // 2023/6/27 デイユース開発 高橋（修正）---
      is_publishing,
      // 2023/2/27 デイユース開発 高橋（修正）---
      local_currency;
    local_currency = jQuery('#local_currency').val();
    ...
    $('.extra_pay_option').each(function () {
      ...
      temp_opt = temp_opt + '|' + $(this).find('.extra_option_value ').val();
      temp_opt = temp_opt + '|' + 0;
      temp_opt = temp_opt + '|' + $(this).find('.extra_option_checkin').val();
      temp_opt = temp_opt + '|' + $(this).find('.extra_option_checkout').val();
      extra_pay_options.push(temp_opt);
    });
    ...
    if (wpestate_check_for_mandatory() || validate_item()) {
      return;
    }
  ...
  $('#edit_prop_1').on('click', function () {
    var ajaxurl, title, max_extra_guest_no, overload_guest, category, action_category, guests, city, country, area, listing_edit, prop_desc, property_admin_area, children_as_guests, instant_booking, aff_link, private_notes, is_publishing;
    title = jQuery('#title').val();
    ...
    $('#prop_category_submit').removeClass('field_err');
    $('#prop_action_category_submit').removeClass('field_err');
      ...
      success: function (data) {
        if (data.edited) {
          ...
          $('#profile_message')
            .empty()
            .append('<div class="login-alert">' + data.response + '<div>');
          var ele_err = data.ele_err;
          ele_err.forEach(function (item) {
            if (item == 'title') {
              $('#title').addClass('field_err');
            }
            if (item == 'category') {
              $('#prop_category_submit').addClass('field_err');
            }
            if (item == 'second_category') {
              $('#prop_action_category_submit').addClass('field_err');
            }
            // if (item == ('ac_cate')) {
            //     $('#prop_action_category_submit').addClass('field_err');
            // }
...
function wpestate_check_for_mandatory(is_check = 0) {
  var reply = '';
  console.log(ajaxcalls_add_vars.mandatory_fields);

  // 2023/2/27 デイユース開発 高橋（追加）---
  if (jQuery('#local_currency').val() == '0') {
    jQuery('#local_currency').addClass('field_err');
    reply = wpestate_build_warning(reply, 'local_currency');
  } else {
    jQuery('#local_currency').removeClass('field_err');
  }
    ...
    ajaxcalls_add_vars.mandatory_fields.forEach(function (field) {
      ...
      if (jQuery('#' + field).length > 0) {
        ...
        if (field === 'title') {
          if (jQuery('#' + field).val() === '') {
            reply = wpestate_build_warning(reply, field_orignal);
            jQuery('#' + field).addClass('field_err');
          }
        }
        ...
        if (jQuery('#' + field).val() === '') {
          ...
          jQuery('#' + field).removeClass('field_err');
        }

        if (field === 'prop_category_submit') {
          if (jQuery('#title').val() === '') {
            // 2023/6/27 デイユース開発 高橋（修正）---
            // reply = 'Hotel name';
            reply = ajaxcalls_add_vars.mandatory_fields_label['title'];
            jQuery('#title').addClass('field_err_123');
            jQuery('.login-alert').append('Home');
          } else {
            jQuery('#title').removeClass('field_err');
          }
        ...
        if (/*field==="prop_category_submit" || */ field === 'prop_action_category_submit') {
          if (jQuery('#title').val() === '') {
            // reply = wpestate_build_warning(reply, field_orignal);
            jQuery('#title').addClass('field_err');
            jQuery('.login-alert').append('Home');
          } else {
            jQuery('#title').removeClass('field_err');
          }
  ...
  // 2023/2/2 デイユース開発 高橋（追加）---
  var error_flg = 0;
  if (jQuery('#edit_prop_price').length) {
    if (jQuery('input').hasClass('extra_option_name')) {
      jQuery('.add_option_wrapper').css('border', 'none');

      jQuery('.extra_pay_option').each(function () {
        if (jQuery(this).find('.extra_option_name').val() === '') {
          jQuery(this).find('.extra_option_name').addClass('field_err');
          error_flg++;
        } else {
          jQuery(this).find('.extra_option_name').removeClass('field_err');
        }
        if (jQuery(this).find('.extra_option_value').val() === '') {
          jQuery(this).find('.extra_option_value').addClass('field_err');
          error_flg++;
        } else {
          jQuery(this).find('.extra_option_value').removeClass('field_err');
        }
        if (jQuery(this).find('.extra_option_checkin').val() === '-1') {
          jQuery(this).find('.extra_option_checkin').addClass('field_err');
          error_flg++;
        } else {
          jQuery(this).find('.extra_option_checkin').removeClass('field_err');
        }
        if (jQuery(this).find('.extra_option_checkout').val() === '-1') {
          jQuery(this).find('.extra_option_checkout').addClass('field_err');
          error_flg++;
        } else {
          jQuery(this).find('.extra_option_checkout').removeClass('field_err');
        }
      });
    } else {
      jQuery('.add_option_wrapper').css('border', '1px solid');
      jQuery('.add_option_wrapper').css('border-color', 'red');

      error_flg++;
    }
    if (error_flg != 0) {
      reply = wpestate_build_warning(reply, 'extra_options');
    }
  }
  ...
  if (reply !== '') {
    jQuery('#profile_message')
      .empty()
      .append('<div class="login-alert alert_err">' + ajaxcalls_add_vars.pls_fill + ': ' + reply + '<div>');
    return true;
  } else {
...
// 2023/3/3 デイユース開発 高橋（追加）---
function validate_item() {
  var err_flag_value = false;
  var err_flag_check_in_out = false;
  var reply_validation = '';
  var inputValue = '';
  var inputCheckIn = '';
  var inputCheckOut = '';

  const error_message_value = ajaxcalls_add_vars.err_msg_value;
  const error_message_value_check_in_out = ajaxcalls_add_vars.err_msg_value_check_in_out;

  jQuery('.extra_pay_option').each(function () {
    inputValue = jQuery(this).find('.extra_option_value').val();
    inputCheckIn = jQuery(this).find('.extra_option_checkin').val();
    inputCheckOut = jQuery(this).find('.extra_option_checkout').val();
    ...
    if (Number(inputCheckIn) >= Number(inputCheckOut)) {
      jQuery(this).find('.extra_option_checkin').addClass('field_err');
      jQuery(this).find('.extra_option_checkout').addClass('field_err');

      err_flag_check_in_out = true;
    } else {
      jQuery(this).find('.extra_option_checkin').removeClass('field_err');
      jQuery(this).find('.extra_option_checkout').removeClass('field_err');
    }
  ...
  if (err_flag_value || err_flag_check_in_out) {
    if (err_flag_value && err_flag_check_in_out) {
      reply_validation = '*' + error_message_value + '<br>*' + error_message_value_check_in_out;
    } else if (err_flag_value && !err_flag_check_in_out) {
      reply_validation = error_message_value;
    } else if (!err_flag_value && err_flag_check_in_out) {
      reply_validation = error_message_value_check_in_out;
    }
    jQuery('#profile_message')
      .empty()
      .append('<div class="login-alert alert_err">' + reply_validation + '<div>');
    return true;
  } else {
    return false; // ready to edit
  }
...
// 2023/3/3 デイユース開発 高橋（追加）---
jQuery('#property_price').on('input', function () {
...
// 2023/3/3 デイユース開発 高橋（追加）---
jQuery('#property_taxes').on('input', function () {
  var inputValue = jQuery(this).val();
  if (/[^0-9]/.test(inputValue)) {
    jQuery(this).val(inputValue.replace(/[^0-9]/g, ''));
  }
...
// 2023/3/3 デイユース開発 高橋（追加）---
jQuery('#city_fee').on('input', function () {
  var inputValue = jQuery(this).val();
  if (/[^0-9]/.test(inputValue)) {
    jQuery(this).val(inputValue.replace(/[^0-9]/g, ''));
  }
...
// 2023/3/3 デイユース開発 高橋（追加）---
jQuery('.extra_pay_option')
  .find('.extra_option_value')
  .on('input', function () {
    var inputValue = jQuery(this).val();
    if (/[^0-9]/.test(inputValue)) {
      jQuery(this).val(inputValue.replace(/[^0-9]/g, ''));
...
// 2023/3/3 デイユース開発 高橋（追加）---
jQuery('#add_option_value').on('input', function () {
  var inputValue = jQuery(this).val();
  if (/[^0-9]/.test(inputValue)) {
    jQuery(this).val(inputValue.replace(/[^0-9]/g, ''));
  }
...
// 2023/3/3 デイユース開発 高橋（追加）---
jQuery('#property_empty').on('input', function () {
  var inputValue = jQuery(this).val();
  if (/[^0-9]/.test(inputValue)) {
    jQuery(this).val(inputValue.replace(/[^0-9]/g, ''));
  }
...
// 2023/3/3 デイユース開発 高橋（追加）---
jQuery('#property_size').on('input', function () {
  var inputValue = jQuery(this).val();
  if (/[^0-9]/.test(inputValue)) {
    jQuery(this).val(inputValue.replace(/[^0-9]/g, ''));
  }
...
// 2023/3/3 デイユース開発 高橋（追加）---
jQuery('#property_rooms').on('input', function () {
  var inputValue = jQuery(this).val();
  if (/[^0-9]/.test(inputValue)) {
    jQuery(this).val(inputValue.replace(/[^0-9]/g, ''));
  }
...
// 2023/3/3 デイユース開発 高橋（追加）---
jQuery('#property_bedrooms').on('input', function () {
  var inputValue = jQuery(this).val();
  if (/[^0-9]/.test(inputValue)) {
    jQuery(this).val(inputValue.replace(/[^0-9]/g, ''));
  }
...
// 2023/3/3 デイユース開発 高橋（追加）---
jQuery('.bedroom_input_val_wrapper')
  .find('.bedroom_input_val')
  .on('input', function () {
    var inputValue = jQuery(this).val();
    if (/[^0-9]/.test(inputValue)) {
      jQuery(this).val(inputValue.replace(/[^0-9]/g, ''));
...
// 2023/3/3 デイユース開発 高橋（追加）---
jQuery('#property_bathrooms').on('input', function () {
  var inputValue = jQuery(this).val();
  if (/[^0-9]/.test(inputValue)) {
    jQuery(this).val(inputValue.replace(/[^0-9]/g, ''));
  }
