...
function wpestate_owner_insert_book(button, pb) {
  'use strict';
  ...
  var front_instants = '';
  if (button.is('#submit_booking_front_instant')) {
    front_instants = 'front_instants';
  }
  ...
  jQuery.ajax({
    ...
    success: function (data) {
      ...
      if (action_function == 'wpestate_ajax_add_booking_instant') {
        ...
        wpestate_booking_request_submit();
        // 2023/6/12 デイユース開発 高橋（追加）---
...
function wprentals_check_booking_valability(button) {
  ...
  book_from = String(moment(book_from).format('YYYY-MM-DD'));
  book_to = String(moment(book_to).format('YYYY-MM-DD'));

  var data_available = jQuery('#start_date').attr('data-available');
  ...
  jQuery.ajax({
    ...
    success: function (data) {
      console.log(data);
      if (data.trim() === 'run') {
        wpestate_owner_insert_book(button, pb);
        jQuery('#booking_form_request_mess').removeClass('no_rooms_available');
      ...
      } else if (data === 'stop array_key_exists') {
        jQuery('#booking_form_request_mess').empty().addClass('book_not_available no_rooms_available').text(control_vars.noroomsavailable);
      } else {
...
function wpestaste_check_in_out_enable_new(in_date, out_date) {
  ...
  jQuery('#' + in_date).daterangepicker(options, function (start, end, label) {
    ...
    end_date = start.format(date_format);

    console.log(start_date);
    console.log(end_date);

    wpestate_setCookie('booking_start_date_cookie', start_date, 1);
    wpestate_setCookie('booking_end_date_cookie', start_date, 1);
  ...
  jQuery('#check_in_widget').on('apply.daterangepicker', function (ev, picker) {
    jQuery(this).val(picker.startDate.format('MM/DD/YYYY') + ' - ' + picker.endDate.format('MM/DD/YYYY'));
  });

  jQuery('#check_in_widget').on('cancel.daterangepicker', function (ev, picker) {
    jQuery(this).val('');
  });

  jQuery('#check_in').on('apply.daterangepicker', function (ev, picker) {
    jQuery(this).val(picker.startDate.format('YYYY/MM/DD'));
    console.log(picker.startDate.format('YYYY/MM/DD'));
    var ptoday = picker.startDate.format('YYYY/MM/DD');
    wpestate_setCookie('booking_start_date_cookie', ptoday, 1);
    wpestate_setCookie('booking_end_date_cookie', ptoday, 1);
  });

  jQuery('#check_in').on('cancel.daterangepicker', function (ev, picker) {
    jQuery(this).val('');
  });

  // jQuery('input[name="check_in"]').daterangepicker();
...
*/
  ...
  $('.advanced_search_map_list_container_trigger input[type="text"]').change(function () {
    if ($(this).attr('id') !== 'search_location' && $(this).attr('id') !== 'check_in' && $(this).attr('id') !== 'search_locationhalf' && $(this).attr('id') !== 'search_locationsidebar' && $(this).attr('id') !== 'search_location' && $(this).attr('id') !== 'search_locationmobile') {
      wpestate_start_filtering_ajax_map(1);
    }
  });
