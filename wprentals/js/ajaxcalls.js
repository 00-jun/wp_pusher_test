...
/// show form booking
////////////////////////////////////////////////////////////////////////////////////////////
function wpestate_show_bk_form(type, ispop) {
  'use strict';

  if (parseInt(type, 10) === 1) {
    jQuery('#ajax_show_frm').show();
    // jQuery('#ajax_login_div').hide();
    // jQuery('#ajax_register_div').hide();
    // jQuery('#forgot-pass-div_mod').hide();
  } else {
    jQuery('#ajax_show_frm').hide();
    // jQuery('#ajax_register_div').hide();
    // jQuery('#forgot-pass-div_mod').hide();
  }
...
function wpestate_register_wd() {
  'use strict';
  var user_pass, user_pass_retype, capthca, user_login_register, user_firstname_register, user_lastname_register, user_email_register, nonce, ajaxurl, user_type, user_phone;
  user_login_register = jQuery('#user_login_register_wd').val();
  // // 2023/1/20 デイユース開発 高橋（追加）---
  user_firstname_register = jQuery('#user_firstname_register_wd').val();
  user_lastname_register = jQuery('#user_lastname_register_wd').val();
  ...
  if (!jQuery('#user_terms_register_wd').is(':checked')) {
    jQuery('#register_message_area_wd')
      .empty()
      .append('<div class="login-alert alert_err">' + control_vars.terms_cond + '</div>');
    return;
  }
  ...
  jQuery.ajax({
    ...
    data: {
      ...
      // 2023/1/20 デイユース開発 高橋（追加）---
      user_firstname_register: user_firstname_register,
      user_lastname_register: user_lastname_register,
    ...
    success: function (data) {
      if (data.register === true) {
        ...
        jQuery('#register_message_area_wd')
          .empty()
          .append('<div class="alert_err login-alert">' + data.message + '</div>');
      }
...
function wpestate_register_wd_mobile() {
  ...
  user_firstname_register = jQuery('#user_firstname_register_wd_mobile').val();
  user_lastname_register = jQuery('#user_lastname_register_wd_mobile').val();
...
function wpestate_login_wd() {
  ...
  jQuery('#login_message_area_wd')
    .empty()
    .append('<div class="login-alert">' + ajaxcalls_vars.login_loading + '</div>');
  jQuery.ajax({
    type: 'POST',
    dataType: 'json',
    url: ajaxurl,
  ...
  jQuery.ajax({
    ...
    success: function (data) {
      if (data.loggedin === true) {
        ...
        jQuery('#login_message_area_wd_mobile')
          .empty()
          .append('<div class="login-alert alert_err">' + data.message + '<div>');
        jQuery('#login_user').val('');
        jQuery('#login_pwd').val('');
      }
    },
...
function wpestate_register() {
  ...
  var user_pass, user_pass_retype, capthca, user_login_register, user_firstname_register, user_lastname_register, user_email_register, nonce, ajaxurl, propid, user_type, user_phone;
  user_login_register = jQuery('#user_login_register').val();

  // 2023/2/2 デイユース開発 高橋（追加）---
  user_firstname_register = jQuery('#user_firstname_register').val();
  user_lastname_register = jQuery('#user_lastname_register').val();
  ...
  if (!jQuery('#user_terms_register_sh').is(':checked')) {
    jQuery('#register_message_area')
      .empty()
      .append('<div class="alert_err login-alert">' + control_vars.terms_cond + '</div>');
    grecaptcha.reset();
    return;
  }
...
function wpestate_register_sh() {
  'use strict';
  var capthca, user_pass, user_pass_retype, user_login_register, user_firstname_register, user_lastname_register, user_email_register, nonce, ajaxurl, propid, user_type, user_phone;
...
jQuery(document).ready(function ($) {
  ...
  $('#update_profile').on('click', function () {
    ...
    var conuntry_mobile = $('#usermobile').attr('data-intltel');

    // 2023/3/3 デイユース開発 高橋（コメントアウト）---
    var selectedcountry = '';

    if (jQuery('.iti--allow-dropdown').length) {
    ...
    var managercontact = $('#manager_contact').val();
    ...
    var data = {
      ...
      conuntry_mobile: selectedcountry.iso2,
      userskype: userskype,
    ...
    var firstname_elm = $('#firstname');
    var secondname_elm = $('#secondname');

    if (firstname_elm.length > 0) {
      data['firstname'] = firstname;
    }

    if (secondname_elm.length > 0) {
    ...
    $.ajax({
      ...
      //     // $('#profile_message').empty().append('<div class="login-alert">' + data.firstname  + '<div>');
      //     if(data.email){
      //         $('.login-alert').append('<p>' + data.email  + '</p>');
      //     }
      //     if(data.firstname){
      //         $('.login-alert').append('<p>' + data.firstname  + '</p>');
      //     }
      //     if(data.secondname){
      //         $('.login-alert').append('<p>' + data.secondname  + '</p>');
      //     }
      //     if(data.userphone){
      //         $('.login-alert').append('<p>' + data.userphone  + '</p>');
      //     }
      //     if(data.updated){
      //         $('.login-alert').append('<p>' + data.updated  + '</p>');
      //     }
      //     // $('.login-alert').append('<p>' + data.secondname  + '</p>');
      //     // $('.login-alert').append('<p>' + data.userphone  + '</p>');
      //     // $('.login-alert').append('<p>' + data.updated  + '</p>');
      ...
      //     //     scrollTop: $("#profile_message").offset().top - 120
      //     // }, 500);
      // },
      // error: function (errorThrown) {
      // }
      ...
      beforeSend: function () {
        $('#profile_message')
          .empty()
          .append('<div class="login-alert fix-bg-h">' + '<p class="loading">' + ajaxcalls_vars.saving + '</p>' + '<div>');
      },
      success: function (data) {
        if (data.email) {
          $('.login-alert').append('<p class="' + data.class + '">' + data.email + '</p>');
        }
        if (data.firstname) {
          $('.login-alert').append('<p class="' + data.class + '">' + data.firstname + '</p>');
        }
        if (data.secondname) {
          $('.login-alert').append('<p class="' + data.class + '">' + data.secondname + '</p>');
        }
        if (data.userphone) {
          $('.login-alert').append('<p class="' + data.class + '">' + data.userphone + '</p>');
        }
        if (data.updated) {
          $('.login-alert').append('<p class="' + data.class + '">' + data.updated + '</p>');
        }
        if (data.err_phone) {
          $('.login-alert').append('<p class="' + data.class + '">' + data.err_phone + '</p>');
        }
        if (data.err_lphone) {
          $('.login-alert').append('<p class="' + data.class + '">' + data.err_lphone + '</p>');
        }
        if (data.err_mphone) {
          $('.login-alert').append('<p class="' + data.class + '">' + data.err_mphone + '</p>');
        }
        if (data.err_fname) {
          $('.login-alert').append('<p class="' + data.class + '">' + data.err_fname + '</p>');
        }
        if (data.err_fname) {
          $('.login-alert').append('<p class="' + data.class + '">' + data.err_lname + '</p>');
        }
        $('html,body').animate({ scrollTop: 0 }, 'fast');
        $([document.documentElement, document.body]).animate(
          {
            scrollTop: $('#profile_message').offset().top - 800,
          },
          500
        );
        $('.loading').hide();
  ...
  ///////////////////////////////////////////////////////////////////////////////////////////
  $('.duplicate-property').on('click', function (event) {
    event.preventDefault();
    var ajaxurl;
    var self = this;
    ajaxurl = ajaxcalls_vars.admin_url + 'admin-ajax.php';
    ...
    $.ajax({
      ...
      url: ajaxurl,
      data: {
        action: 'wpestate_duplicate_property',
        prop_id: property_id,
      },
      success: function (data) {
        var this_url = $(self).attr('href');
        var location_url = this_url.replace(/edit=.*action/, 'edit=' + data + '&action');
        $(self).attr('href', location_url);
