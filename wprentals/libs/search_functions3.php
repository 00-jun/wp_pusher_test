...
if( !function_exists('wpestate_argumets_builder') ):
    function wpestate_argumets_builder($input,$is_half=''){
        ...
        if( isset($input['check_in'])){
            ...
            $wpestate_book_from = date("Y-m-d", strtotime($wpestate_book_from) );
        }
        ...
        // }
        if( isset($input['check_in'])){
            $wpestate_book_to        =  sanitize_text_field( $input['check_in'] );
            $wpestate_book_to = date("Y-m-d", strtotime($wpestate_book_to) );
        }
    ...
    function wpestate_get_action_select_list_4all($args,$taxonomy){
        ...
        if($categ_select_list===false){
            ...
            foreach ($categories as $categ) {
                ...
                $term_post_count = get_term_post_count_by_type($categ->term_id,$taxonomy,'estate_property');
                $term_post_count = (($counter - $term_post_count) > 0) ? ($counter - $term_post_count) : 0;
                ...
                // $categ_select_list     .=   '<li role="presentation" data-value="'.esc_attr($categ->slug).'">'. ucwords ( urldecode( $categ->name ) ).' ('.$term_post_count.')'.'</li>';
