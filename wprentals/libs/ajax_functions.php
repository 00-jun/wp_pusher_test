...
if (!function_exists('wpestate_admin_activate_reservation_fee')):
    function wpestate_admin_activate_reservation_fee()
    {
        ...
        $property_id        =   intval($_POST['prop_id']);
        $book_from          =   get_post_meta($booking_id, 'booking_from_date', true);
        $book_from = '20'.$book_from;
        ...
        $plan_count_values = array_count_values(wpestate_get_booking_plans_id($property_id));

        if ($plan_count_values[$from_date_unix] > ($property_emptys-1)) {
            print'1';
            die();
        }else{
            print'0';
...
////////////////////////////////////////////////////////////////////////////////
/// Ajax  add booking request  function
////////////////////////////////////////////////////////////////////////////////

add_action( 'wp_ajax_wpestate_ajax_add_booking_request', 'wpestate_ajax_add_booking_request' );
if( !function_exists('wpestate_ajax_add_booking_request') ):
    function wpestate_ajax_add_booking_request(){

        check_ajax_referer( 'wprentals_booking_request_submit_nonce', 'security' );
        ...
        update_post_meta($bookid, 'custom_price_array',$booking_array['custom_price_array']);
...
if (!function_exists('wpestate_disable_listing')):
    ...
    function wpestate_disable_listing()
    {
        ...
        $id_en = apply_filters( 'wpml_object_id', $prop_id, 'post', TRUE, 'en' );
        $original_post = array(
            'ID'            => $id_en,
            'post_status'   => $new_status
        );
        $id_ja = apply_filters( 'wpml_object_id', $prop_id, 'post', TRUE, 'ja' );
        $trans_post = array(
            'ID'            => $id_ja,
            'post_status'   => $new_status
        );
...
if (!function_exists('wpestate_ajax_filter_invoices')):
    function wpestate_ajax_filter_invoices()
    {
        ...
        echo json_encode(array('results'=>$templates,'invoice_issued'=>wpestate_show_price_custom_invoice($inv_id,$total_issued), 'invoice_confirmed'=> wpestate_show_price_custom_invoice($inv_id,$total_confirmed) ));
...
if (!function_exists('wpestate_ajax_register_form')):
    ...
    function wpestate_ajax_register_form()
    {
        ...
        // 2023/2/3 デイユース開発 高橋（追加）---
        $user_firstname =   trim(sanitize_text_field($_POST['user_firstname_register']));
        $user_lastname  =   trim(sanitize_text_field($_POST['user_lastname_register']));
        $user_phone     =   trim(sanitize_text_field($_POST['user_phone']));


        // 2023/2/3 デイユース開発 高橋（修正）---
        if ($user_name=='') {
            echo json_encode(array('register'=>false,'message'=>esc_html__('User name field is empty!', 'wprentals')));
            exit();
        }
        ...
        // 2023/2/3 デイユース開発 高橋（追加）---
        if ($user_firstname=='') {
            echo json_encode(array('register'=>false,'message'=>esc_html__('Name is empty!', 'wprentals')));
            exit();
        }

        // 2023/3/31 デイユース開発 高橋（追加）---
        if (!ctype_alpha($user_firstname)){
            echo json_encode(array('register'=>false,'message'=>esc_html__('Name must be half-width alphabetical characters!', 'wprentals')));
            exit();
        }

        //  2023/2/3 デイユース開発 高橋（追加）---
        if ($user_lastname=='') {
            echo json_encode(array('register'=>false,'message'=>esc_html__('Surname is empty!', 'wprentals')));
            exit();
        }

        // 2023/3/31 デイユース開発 高橋（追加）---
        if (!ctype_alpha($user_lastname)){
            echo json_encode(array('register'=>false,'message'=>esc_html__('Surname must be half-width alphabetical characters!', 'wprentals')));
            exit();
        }

        // 2023/2/3 デイユース開発 高橋（修正）---
        if ($user_email=='') {
            echo json_encode(array('register'=>false,'message'=>esc_html__('Email field is empty!', 'wprentals')));
            exit();
        }
        ...
        if (!$user_id and email_exists($user_email) == false) {
            ...
            if (is_wp_error($user_id)) {
                ...
                // 2023/2/3 デイユース開発 高橋（追加）---
                if (isset($_POST['user_firstname_register'])) {
                    update_user_meta($user_id, 'first_name', sanitize_text_field($_POST['user_firstname_register']));
                }

                if (isset($_POST['user_lastname_register'])) {
                    update_user_meta($user_id, 'last_name', sanitize_text_field($_POST['user_lastname_register']));
                }
...
if (!function_exists('wpestate_ajax_update_profile')):
    ...
    function wpestate_ajax_update_profile()
    {
        ...
        $hotelname               = sanitize_text_field($_POST['hotelname']);
        $managername             = sanitize_text_field($_POST['managername']);
        $managercontact          = sanitize_text_field($_POST['managercontact']);
        $department              = sanitize_text_field($_POST['department']);

        $conuntry_mobile              = sanitize_text_field($_POST['conuntry_mobile']);
        ...
        if ($current_user->user_email != $useremail) {
            ...
            if ($user_id) {
                $result['email'] = __('The email was not saved because it is used by another user. ', 'wprentals');
                $error = 1;
                $result['class'] = "error-h";
            } elseif ($useremail=='') {
                $result['email'] = __('The email field cannot be blank. ', 'wprentals');
                $error = 2;
                $result['class'] = "error-h";
            } elseif (filter_var($useremail, FILTER_VALIDATE_EMAIL) === false) {
                $result['email'] = __('The email doesn\'t look right !', 'wprentals');
                $error = 3;
                $result['class'] = "error-h";
            } else {
                $args = array(
        ...
        if (isset($_POST['firstname'])) {
            if($firstname == ""){
                $result['firstname'] = esc_html__('Name cannot be left blank', 'wprentals');
                $error = 4;
                $result['class'] = "error-h";
            };
        }
        ...
        if (isset($_POST['secondname'])) {
            if($secondname == ""){
                $result['secondname'] = __('Surname cannot be left blank', 'wprentals');
                $error = 5;
                $result['class'] = "error-h";
            }
        }
        ...
        // if($userphone == ""){
        //     $result['userphone'] = esc_html__('Phone number can not be left blank', 'wprentals');
        //     $error = 6;
        //     $result['class'] = "error-h";
        // }
        ...
        //        $user_phone = get_user_meta ( $user->ID, 'phone' );
        //        $user_fname = get_user_meta ( $user->ID, 'first_name' );
        //        $user_lname = get_user_meta ( $user->ID, 'last_name' );
        //        if( $user_phone[0] == $userphone && $user_phone[0] !='' ) {
        //           $error = 7;
        //           $result['err_phone'] = __('Phone number already exists', 'wprentals');
        //           $result['class'] = "error-h";
        //        }

        //        if(strlen($userphone) < 10 || strlen($userphone) > 10) {
        //             $error = 10;
        //             $result['err_lphone'] = __('Phone number only 10 characters', 'wprentals');
        //             $result['class'] = "error-h";
        //        }

        //        // if($user_fname[0] == $firstname){
        //        //    $error = 8;
        //        //    $result['err_fname'] = __('Firstname already exists', 'wprentals');
        //        //  }

        //        //  if($user_lname[0] == $secondname){
        //        //     $error = 9;
        //        //     $result['err_lname'] = __('Last Name already exists', 'wprentals');
        //        //  }
        //     };
        // }

        // 2023/2/3 デイユース開発 高橋（コメントアウト）---
        // if ($mb_phone != $usermobile) {
        //     if(strlen($usermobile) < 10 || strlen($usermobile) > 10) {
        //         $error = 11;
        //         $result['err_mphone'] = __('Phone number only 10 characters', 'wprentals');
        //         $result['class'] = "error-h";
        //     }
        // }
        ...
        if(!$error){
            ...
            update_user_meta($userID, 'mobile_country', $conuntry_mobile) ;
            ...
            update_user_meta($userID, 'hotel_name', $hotelname);
            update_user_meta($userID, 'manager_name', $managername);
            update_user_meta($userID, 'manager_contact', $managercontact);
            ...
            wpestate_update_user_agent($agent_id, $firstname, $secondname, $useremail, $userskype, $profile_image_url, $usermobile, $about_me, $profile_image_url_small, $userfacebook, $usertwitter, $userlinkedin, $userpinterest, $live_in, $i_speak, $login_name, $payment_info, $youtube, $instagram, $userwebsite, $managername, $managercontact,$hotelname) ;

            $result['updated'] = __('Profile updated', 'wprentals');
        }
...
if (!function_exists('wpestate_update_user_agent')):
    function wpestate_update_user_agent($agent_id, $firstname, $secondname, $useremail, $userskype, $profile_image_url, $usermobile, $about_me, $profile_image_url_small, $userfacebook, $usertwitter, $userlinkedin, $userpinterest, $live_in, $i_speak, $login_name, $payment_info, $youtube, $instagram, $userwebsite, $managername, $managercontact,$hotelname)
    {
        ...
        update_post_meta($agent_id, 'about_me',$about_me);
        ...
        update_post_meta($agent_id, 'manager_name', $managername);
        update_post_meta($agent_id, 'manager_contact', $managercontact);
        update_post_meta($agent_id, 'agent_hotel_name', $hotelname);
