...
function wpestate_generate_trip_details($property_id,$invoice_id,$bookid,$mode=''){
    ...
    print '<div class="trip_details_wrapper"><div class="trip_details_container" style="'.esc_attr($mode_wrapper_trip_details_container).'" >';
        ...
        print wpestate_convert_dateformat_reverse($booking_from_date);
        ...
        print '<h5 style="'.esc_attr($mode_wrapper_h5).'" >'.esc_html__('Accommodation','wprentals').'</h5>';
        ...
        // 2023/2/2 デイユース開発 高橋（追加）
        if(isset($owner_details['hotel_name']) && $owner_details['hotel_name']!=''){
            if(isset($owner_details['hotel_name'])) {
                print $owner_details['hotel_name'];
            }
            print '<br>';
        }
    ...
    if ($agent_id!=0){        
        ...
        if( $agent_selection->have_posts() ){
               ...
               while ($agent_selection->have_posts()): $agent_selection->the_post();  
                    ...
                    //2023/2/4 デイユース開発 高橋 (修正) -->
                    $return_agent['hotel_name']        = esc_html(get_the_author_meta( 'hotel_name' , $owner_id ) );
...
if (!function_exists('wpestate_show_booking_form')):
    ...
    function wpestate_show_booking_form($post_id, $wpestate_options = '', $favorite_class = '', $favorite_text = '', $is_shortcode = '') {
        ...
        ?>
            ...
            <!--2023/2/2 デイユース開発 高橋（コメントアウト）-->
            <!-- <div class="available_rooms_label"><span class="booking_details_title">Available rooms: </span> <span class="invoice_list_empty"><?php //echo get_post_meta($post_id, 'property_empty', true); ?></span></div> -->
        ...
        ?>
            ...
            <div class="has_calendar calendar_icon">
                <input type="text" id="start_date" placeholder="<?php echo wpestate_show_labels('check_in', $rental_type); ?>"  class="form-control calendar_icon" size="40" name="start_date"
                       data-available="<?php echo get_post_meta($post_id, 'property_empty', true); ?>" value="<?php
        ...
        ?>">
            ...
            <?php if (isset($_GET['room_id'])): ?>
                <input type="hidden" id="listing_edit" name="listing_edit" value="<?php print intval($_GET['room_id']); ?>" />
            <?php else: ?>
        ...
        <?php wpestate_show_booking_button($post_id); ?>
            ...
            <div class="third-form-wrapper">
                <!--2023/2/19 デイユース開発 清水（追加）クラス追加-->
                <div class="reservation_buttons favorites_button">
        ...
        if ($is_shortcode == 1) {
            ...
            <div class="modal fade" id="instant_booking_modal" tabindex="-1" aria-labelledby="myModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-header">
                            ...
                            <!-- 2023/8/2 デイユース開発 高橋（修正） -->
                            <h4 class="modal-title" id="myModalLabel"><?php esc_html_e('Review the booking info and click Submit button', 'wprentals'); ?></h4>
             ...
             <!-- 2023/2/2 デイユース開発 高橋（追加   ）----->
             <div class="modal fade" id="already_filled_modal" tabindex="-1" aria-labelledby="myModalLabel" aria-hidden="true">
             <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal_message">
                        <h2><?php esc_html_e( 'Sorry, this reservation is already filled.','wprentals'); ?></h2>
                    </div>
                    <div class="modal_back_btn">
                    <?php 
                        print' <button class="vc_button wpb_btn-info" id="reservation_filled_back" onclick="window.location.reload()">'.esc_html__( 'Back','wprentals').'</button>';
                        ?>
                    </div>
                    </div>
                </div>
        ...
        if ($affiliate_link == '') {
            ...
            if ($instant_booking == 1) {
                ...
                <div id="submit_booking_front_instant_wrap">
                    <input type="submit" id="submit_booking_front_instant"  data-maxguest="<?php print esc_attr($max_guest); ?>" 
...
if (!function_exists('wpestate_booking_guest_explanations')):
    ...
    function wpestate_booking_guest_explanations($post_id) {
        ...
        if($guests!='' && wprentals_get_option('wp_estate_custom_guest_control','') =='yes' ){
            ...
            //2023/2/12 デイユース開発 高橋（修正）
            $return.= sprintf( _n( 'Adult: %s', 'Adults: %s', $guests, 'wprentals' ), number_format_i18n( $guests ) );
            $return.= ', '.sprintf( _n( 'Child: %s', 'Children: %s', $childs, 'wprentals' ), number_format_i18n( $childs ) );
            $return.= ', '.sprintf( _n( 'Infant: %s', 'Infants: %s', $infants, 'wprentals' ), number_format_i18n( $infants ) );
            $return.=')';
        ...
        if ($wpestate_property_unit_slider == 'yes') {
            ...
            foreach ($post_attachments as $attachment) {
                ...
                $slides .= '<div class="item lazy-load-item">
                                        <a href="' . esc_url($link) . '" onclick="return setdateCookie(this)" target="' . esc_attr(wprentals_get_option('wp_estate_prop_page_new_tab', '')) . '" data-listid="'.$post_id.'" ><img  data-lazy-load-src="' . esc_attr($preview[0]) . '" alt="' . esc_attr($attachment->title) . '" class="img-responsive" /></a>
                                    </div>';
            }// end foreach
                ...
                <div id="property_unit_carousel_' . esc_attr($unique_prop_id) . '" class="carousel property_unit_carousel slide  " data-ride="carousel" data-interval="false">
                    <div class="carousel-inner">
                        <div class="item active">
                            <a href="' . esc_url($link) . '" onclick="return setdateCookie(this)" target="' . esc_attr(wprentals_get_option('wp_estate_prop_page_new_tab', '')) . '" data-listid="'.$post_id.'" >' . $thumb_prop . '</a>
                        </div>
                        ' . $slides . '
            ...
            print '<a href="' . esc_url($link) . '" onclick="return setdateCookie(this)" target="' . esc_attr(wprentals_get_option('wp_estate_prop_page_new_tab', '')) . '" data-listid="'.$post_id.'" >' . trim($thumb_prop) . '</a>';
        }
...
if (!function_exists('wpestate_booking_mark_confirmed')):
    ...
    function wpestate_booking_mark_confirmed($booking_id, $invoice_id, $userId, $depozit, $user_email, $is_stripe = 0) {
        ...
        $front_instants = false;
        if ($booking_status != 'confirmed') {
            $front_instants = false;
            update_post_meta($booking_id, 'booking_status', 'confirmed');
            update_post_meta($booking_id, 'front_instants', $front_instants);
        ...
        if ($woo_double_check != 1) {
            ...
            ob_end_clean();
             //2023/2/12 デイユース開発 高橋（修正）
            wpestate_send_emails($user_email, esc_html__('【DayDreamHub】旅行の詳細   [DayDreamHub] Your Trip Details','wprentals'), $message );
...
if (!function_exists('wpestate_show_extra_options_booking')):
    ...
    function wpestate_show_extra_options_booking($post_id) {
        ...
        // 2023/2/29 デイユース開発 高橋（修正）---
        // $wpestate_currency = esc_html(wprentals_get_option('wp_estate_currency_label_main', ''));
        $wpestate_currency = esc_html ( get_post_meta($post_id, 'local_currency', true) );

        $wpestate_where_currency = esc_html(wprentals_get_option('wp_estate_where_currency_symbol', ''));
        $extra_pay_options = ( get_post_meta($post_id, 'extra_pay_options', true) );
        // var_dump($extra_pay_options);
        if (is_array($extra_pay_options)) {
            foreach ($extra_pay_options as $key => $extra_options) {
                // 2023/3/12 デイユース開発 高橋（修正）
                print'<div class="cost_row cost_row_extra wpestate_show_extra_options_booking"  data-value_add="' . esc_attr($extra_options[1]) .  '" data-value_how="' . esc_attr($extra_options[2]) . '" data-value_name="' . $extra_options[0] . '" data-value_time="' . $checkin_out_time[$extra_options[3]].' - '.$checkin_out_time[$extra_options[4]] . '" >';
                print '<div class="cost_explanation"> <input type="radio" data-key="' . esc_attr($key) . '" class="form-control" value="1" name="checkbox"  /> ' . $extra_options[0] .' '.$checkin_out_time[$extra_options[3]].' - '.$checkin_out_time[$extra_options[4]].' </div>';
                print '<div class="cost_value"><div class="cost_value_show">' . wpestate_show_price_booking($extra_options[1], $wpestate_currency, $wpestate_where_currency, 1) . ' </div></div>';
                print '</div>';
            }
            ...
            foreach ($comments as $comment) :
                ...
                $review_templates .= ' <span class="ratings-star">(' . wpestate_get_star_total_value(wpestate_get_star_total_rating($rating)) . ' ' . esc_html__('of', 'wprentals') . ' 5)</span>
                            ...
                            <div class="review-content">
                                ...
                                ' . esc_html__('Posted on ', 'wprentals') . ' ' . get_comment_date('Y\/m\/d', $comment->comment_ID) . '
                                </div>
                            </div>
...
if (!function_exists('wpestate_show_price_custom')):
    ...
    // 2023/2/28 デイユース開発 高橋（追加）$post_id---
    function wpestate_show_price_custom($post_id,$price) {
        $price_label = '';
        $wpestate_currency = esc_html(wprentals_get_option('wp_estate_currency_label_main', ''));
        $wpestate_where_currency = esc_html(wprentals_get_option('wp_estate_where_currency_symbol', ''));
        $th_separator = wprentals_get_option('wp_estate_prices_th_separator', '');
...
if (!function_exists('wpestate_show_price_custom_invoice')):
    ...
    // 2023/2/28 デイユース開発 高橋（追加）$post_id---
    function wpestate_show_price_custom_invoice($post_id,$price) {
        $price_label = '';
...
if (!function_exists('wpestate_check_booking_valability')):
    ...
    function wpestate_check_booking_valability($wpestate_book_from, $wpestate_book_to, $listing_id) {
        ...
        $property_available = floatval(get_post_meta($listing_id, 'property_empty', true));
        ...
        $reservation_plan_array = wpestate_get_booking_plans_room($listing_id);

        if ($from_date_unix === $to_date_unix) {
            if(array_count_values($reservation_plan_array)[$from_date_unix] > ($property_available-1)) {
                return false;
            }

            // $property_notbooking = get_post_meta($listing_id, 'property_notbooking',true);

            // if (!is_array($property_notbooking) && $property_notbooking == '') {
            //     $property_notbooking = [];
            // }

            // if (in_array($from_date_unix,$property_notbooking)) {
            //     return false;
            // }
...
if (!function_exists('wpestate_get_review_fields')) {
    ...
    function wpestate_get_review_fields() {
        $fields = array(
            ...
            'fields' => array(
                'accuracy' => esc_html__('Staff / Service', 'wprentals'),
                'communication' => esc_html__('Facilities', 'wprentals'),
                'cleanliness' => esc_html__('Cleanliness', 'wprentals'),
                'location' => esc_html__('Location', 'wprentals'),
                'check_in' => esc_html__('Check-In', 'wprentals'),
                'value' => esc_html__('Price', 'wprentals'),
            )
        );
...
if (!function_exists('wpestate_calculate_property_rating')) {
    ...
    function wpestate_calculate_property_rating($property_id) {
        ...
        /*****
            Anton Add review for orther languages
        *****/
        $post_lang = apply_filters( 'wpml_post_language_details', NULL, $property_id );
        if( $post_lang['language_code'] == 'en' ) {
            $trans_id = apply_filters( 'wpml_object_id', $property_id, 'post', false, 'ja' );
            update_post_meta($trans_id, 'property_stars', $star_rating);
            update_post_meta($trans_id, 'abc', '25');
        } else {
            $trans_id = apply_filters( 'wpml_object_id', $property_id, 'post', false, 'en' );
            update_post_meta($trans_id, 'property_stars', $star_rating);
            update_post_meta($trans_id, 'abc', '30');
        };
        return $star_rating;
    }
...
if (!function_exists('wpestate_return_all_fields')):
    ...
    function wpestate_return_all_fields($is_mandatory = 0) {
        ...
        $all_submission_fields = $all_mandatory_fields = array(
            // 2023/6/27 デイユース開発 高橋（修正）---
            // 'title' => __('Hotel name', 'wprentals'),
            ...
            // 2023/6/27 デイユース開発 高橋（修正）---
            // 'prop_category_submit' => __('Category', 'wprentals'),
            'prop_category_submit' => __('Specialities', 'wprentals'),
            'prop_action_category_submit' => __('Listed In/Room Type', 'wprentals'),
            // 2023/6/27 デイユース開発 高橋（修正）---
            // 'property_city_front' => __('Property/Item City', 'wprentals'),
            'property_city_front' => __('State / Province', 'wprentals'),
            ...
            // 2023/2/26 デイユース開発 清水（修正）---
            'property_price' => __('Recommended plan', 'wprentals'),
            ...
            // 2023/2/2 デイユース開発 高橋（追加）---
            'extra_options' => __('Plans', 'wprentals'),
            // 2023/2/27 デイユース開発 高橋（追加）---
            'local_currency' => __('Local Currency', 'wprentals'),
...
if (!function_exists('wpestate_show_mandatory_fields')):
    ...
    function wpestate_show_mandatory_fields() {
        ...
        if (is_array($mandatory_fields) && !empty($mandatory_fields)) {
            ...
            _e('These fields are mandatory: Hotel name, ', 'wprentals');
...
if (!function_exists('wpestate_show_labels')):
    ...
    function wpestate_show_labels($label, $type, $book_type = '') {
        ...
        $labels = array(
            ...
            'price_label' => array(
                // 2023/2/26 デイユース開発 清水（修正）---
                '0' => esc_html__('Recommended plan ', 'wprentals'),
                '1' => esc_html__('Price per day', 'wprentals'),
                '2' => esc_html__('Price per hour', 'wprentals'),
            ),
            'price_week_label' => array(
                // 2023/2/26 デイユース開発 清水（修正）---
                '0' => sprintf(esc_html__('Recommended plan  (%sd+)', 'wprentals'), $week_days_no),
            ...
            'price_month_label' => array(
                // 2023/2/26 デイユース開発 清水（修正）---
                '0' => sprintf(esc_html__('Recommended plan  (%sd+)', 'wprentals'), $month_days_no),
            ...
            'price_week_label_ext' => array(
                // 2023/2/26 デイユース開発 清水（修正）---
                '0' => sprintf(esc_html__('Recommended plan  if the item is rented for more than %s nights or more than %s nights', 'wprentals'), $week_days_no, $month_days_no),
            ...
            'early_bird' => array(
                // 2023/2/26 デイユース開発 清水（修正）---
                '0' => esc_html__('Early Bird Discount - in % from the Recommended plan ', 'wprentals'),
...
////////////////////////////////////////////////////////////////////////////////
// save array with bookng
////////////////////////////////////////////////////////////////////////////////
if (!function_exists("wpestate_get_booking_plans_room")):
function wpestate_get_booking_plans_room($listing_id,$on_blank=''){
    $args=array(
        'post_type'        => 'wpestate_booking',
        'post_status'      => 'any',
        'posts_per_page'   => -1,
    ...
    $reservation_plan_array          =   get_post_meta($listing_id, 'booking_plans_room',true);
    
    if( !is_array($reservation_plan_array) || $reservation_plan_array=='' ){
        $reservation_plan_array  =   array();
    }
    ...
    if ($booking_selection->have_posts()){    
        ...
        while ($booking_selection->have_posts()): $booking_selection->the_post();
            ...
            $reservation_plan_array[] = $from_date_unix;
            $from_date_unix                     =   $from_date->getTimestamp(); 
        endwhile;
...
////////////////////////////////////////////////////////////////////////////////
// save array with booking id
////////////////////////////////////////////////////////////////////////////////
if (!function_exists("wpestate_get_booking_plans_id")):
function wpestate_get_booking_plans_id($listing_id){
    ...
    $curent_lang = apply_filters( 'wpml_current_language', NULL );
    $listing_id1 = get_id_translations($listing_id, $curent_lang);
    ...
    $args=array(
        ...
        'meta_query' => array(
            array(
                'key'       => 'booking_id',
                'value'   => array( $listing_id, $listing_id1 ),
                'compare' => 'IN'
            ),
        )
    ...
    if ($booking_selection->have_posts()){    
        $reservation_plan_array  =   array();
        while ($booking_selection->have_posts()): $booking_selection->the_post();
            $front_instants = get_post_meta(get_the_ID(), 'front_instants', true);


            $pid                =   get_the_ID();
            // 2023/6/11 デイユース開発 高橋（追加）---
            $booking_status     =   get_post_meta($pid, 'booking_status', true);
            if(!($booking_status == 'canceled' || $booking_status == 'request_canceled')){
                $fromd              =   esc_html(get_post_meta($pid, 'booking_from_date', true));
                $tod                =   esc_html(get_post_meta($pid, 'booking_to_date', true));
                $unix_time_start    =   strtotime ($fromd);
                $unix_time_end      =   strtotime ($tod);
                ...
                if ($front_instants=='' || $front_instants == '0') {
                    $reservation_plan_array[get_the_ID()] = $from_date_unix;
                }
...
function remove_reservation_plan($array, $reservation_date) {
...
function get_term_post_count_by_type($term,$taxonomy,$type){
  ...
  $args = array( 
   ...
   foreach ($ps as $key => $value) {
    $tpost = get_post($value->ID);
    if ($tpost->post_status=='disabled') {
        $tpost_count[] = $value->ID;
    }
   }
   ...
   // if (count($tpost_count) > 0){
   //     return count($tpost_count);
   // }else{
   //     return 0;
   // }
}
...
function check_booking_plan_valability($listing_id) {
    $larray = wpestate_get_booking_plans_id($listing_id) ? wpestate_get_booking_plans_id($listing_id) : [];
    // $lhash = array_flip_multiple($larray);
    // $chash = array_filter(array_count_values($larray), function($v) { return $v > 0; });
    // $vals = array_count_values($chash);
    // return $lhash;

    $hash = array_flip_multiple($larray);
...
function get_id_translations($post_id, $curent_lang){
    if ($curent_lang == 'en') {
        $other_lang = 'ja';
    } else {
        $other_lang = 'en';
    }
    $translationsID = apply_filters( 'wpml_object_id', $post_id, 'estate_property', FALSE, $other_lang );
    return $translationsID;
}


//2023/3/8 デイユース開発 高橋（追加）
add_action('save_post', 'custom_update_estate_agent', 2, 3);

function custom_update_estate_agent($post_id, $post, $update){
    ...
    $user_id      = wp_kses($_POST['user_agent_id'],$allowed_html);
    $email      = wp_kses($_POST['agent_email'],$allowed_html);
    $mobile     = wp_kses($_POST['agent_mobile'],$allowed_html);
    $url        = wp_kses($_POST['agent_website'],$allowed_html);
    ...
    $desc_contents = esc_html(get_the_content($post_id));
    $desc       = wp_kses($_POST['content'],$allowed_html);
    
    $agent_hotel_name  = wp_kses($_POST['agent_hotel_name'],$allowed_html);
    $about_me = wp_kses($_POST['about_me'],$allowed_html);
    $live_in    = wp_kses($_POST['live_in'],$allowed_html);
    $manager_name    = wp_kses($_POST['manager_name'],$allowed_html);
    $manager_contact    = wp_kses($_POST['manager_contact'],$allowed_html);

    update_post_meta( $post_id, 'agent_email' , $email) ;
    update_post_meta( $post_id, 'agent_mobile' , $mobile) ;
    update_post_meta( $post_id, 'agent_website' , $url) ;
    update_post_meta( $user_id, 'url', $url) ;
    update_post_meta( $post_id, 'agent_facebook', $facebook) ;
    update_post_meta( $post_id, 'agent_twitter', $twitter) ;
    update_post_meta( $post_id, 'agent_instagram', $instagram) ;
    update_post_meta( $post_id, 'agent_youtube', $youtube) ;
    update_post_meta( $post_id, 'agent_hotel_name', $agent_hotel_name) ;
    update_post_meta( $post_id, 'about_me',$about_me);
    update_post_meta( $post_id, 'live_in', $live_in) ;
    update_post_meta( $post_id, 'manager_name', $manager_name) ;
    update_post_meta( $post_id, 'manager_contact', $manager_contact) ;

    if ($desc_contents != $desc) {
        $about_me = $desc;
    }

    if ($desc_contents != $about_me) {
        $about_me = $about_me;
    } 
