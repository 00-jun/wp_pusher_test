...
if( !function_exists('wpestate_ajax_check_booking_valability_on_invoice') ):
    ...
    exit();
        ...
        $data_available = get_post_meta($listing_id, 'property_empty', true);
        ...
        while ($from_date_unix < $to_date_unix){
            ...
            if(array_count_values($reservation_plan_array)[$from_date_unix] > ($data_available-1)) {
                print 'stop';
                die();
            }
        }
...
if( !function_exists('wpestate_ajax_check_booking_valability') ):
    function wpestate_ajax_check_booking_valability(){
        ...
        $data_available = floatval($_POST['data_available']);
        ...
        $plan_count_values = array_count_values(wpestate_get_booking_plans_id($listing_id));

        if ($plan_count_values[$from_date_unix] > ($data_available-1)) {
            print 'stop array_key_exists';
            die();
        }
        ...
        if($wprentals_is_per_hour==2){
            ...
            //   die();
            // }
            if(wprentals_check_booking_plan_overlap_reservations($from_date,$from_date_unix,$to_date_unix,$reservation_plan_array,$data_available)){
              print 'stop';
              die();
            }
        }
...
function wprentals_check_booking_plan_overlap_reservations($from_date,$from_date_unix,$to_date_unix,$reservation_plan_array,$data_available){
    $plan_count_values = array_count_values(wpestate_get_booking_plans_id($listing_id));
...
if( !function_exists('wpestate_delete_booking_request') ):
    function wpestate_delete_booking_request(){
        ...
        $reservation_plan_array = wpestate_get_booking_plans_room($lisiting_id);
        update_post_meta($lisiting_id, 'booking_plans_room', $reservation_plan_array);
        ...
        if($wprentals_is_per_hour==2){
            ...
            $reservation_plan_arrays = remove_reservation_plan($reservation_plan_array, strtotime($fromd));

        }

        update_post_meta($lisiting_id, 'booking_dates', $reservation_array);

        update_post_meta($lisiting_id, 'booking_plans_room', $reservation_plan_arrays);

        // 2023/6/11 デイユース開発 高橋（修正）---
        // wp_delete_post($bookid);
        update_post_meta($bookid, 'booking_status', 'request_canceled');
...
if( !function_exists('wpestate_cancel_own_booking') ):
    function wpestate_cancel_own_booking(){
        ...
        // 2023/6/11 デイユース開発 高橋（修正）---
        update_post_meta($bookid, 'booking_status', 'canceled');
        // wp_delete_post($bookid);
        update_post_meta($listing_id, 'booking_dates', $reservation_array);

        update_post_meta($listing_id, 'booking_plans_room', $reservation_plan_arrays);

        $property_emptys = floatval ( get_post_meta($listing_id, 'property_empty', true));
        $bookked_rooms = floatval ( get_post_meta($listing_id, 'booked_rooms', true));

        if ((($property_emptys - $bookked_rooms) >= 0) && ($bookked_rooms > 0)) {
            $bookked_rooms=$bookked_rooms-1;
            $booking_list_empty = $property_emptys - $bookked_rooms;
            update_post_meta($listing_id, 'booked_rooms', $bookked_rooms);
            update_post_meta($bookid, 'booking_list_empty', $booking_list_empty);
        }
...
// 2023/7/2 デイユース開発 高橋（function追加）---
////////////////////////////////////////////////////////////////////////////////
/// Cancel booking by user
////////////////////////////////////////////////////////////////////////////////

add_action('wp_ajax_wpestate_cancel_booking_by_user', 'wpestate_cancel_booking_by_user' );
if( !function_exists('wpestate_cancel_booking_by_user') ):
    function wpestate_cancel_booking_by_user(){
        //scheck -should split
        check_ajax_referer( 'wprentals_booking_confirmed_actions_nonce', 'security' );
        $current_user = wp_get_current_user();
        $userID                         =   $current_user->ID;
    ...
    //     $receiver          =   get_userdata($user_id);
    //     $receiver_name     =   $receiver->user_login;
    //     $to_id      =   wpse119881_get_author($property_id);
    //     $to_userdata=   get_userdata($to_id);
    //     $to_email   =   $to_userdata->user_email;
...
// 2023/6/11 デイユース開発 高橋（function追加）---
////////////////////////////////////////////////////////////////////////////////
/// Complete booking
////////////////////////////////////////////////////////////////////////////////


add_action('wp_ajax_wpestate_complete_booking', 'wpestate_complete_booking' );
if( !function_exists('wpestate_complete_booking') ):
    function wpestate_complete_booking(){
        //scheck
        check_ajax_referer( 'wprentals_booking_confirmed_actions_nonce', 'security' );
        $current_user = wp_get_current_user();
        $userID         =   $current_user->ID;
        ...
        // check if correct post types
        if( get_post_type($listing_id)!='estate_property' || get_post_type($bookid)!='wpestate_booking'){
          exit('you don\'t have the right to comalete this by by');
        }

        // check if listing has the same owner
        $the_post= get_post( $listing_id);
        if( $current_user->ID != $the_post->post_author ) {
            exit('you don\'t have the right to comalete this');
        }

        // check if booking belogins to approved listing
        $property_id         =   get_post_meta($bookid, 'booking_id', true);
        if($property_id!=$listing_id){
            exit('you don\'t have the right to comalete this2');
        }

        
        $booking_status     =   get_post_meta($bookid, 'booking_status', true);
        if($booking_status=='confirmed'){
            update_post_meta($bookid, 'booking_status', 'completed');
        }
...
add_action('wp_ajax_wpestate_owner_cancel_own_booking', 'wpestate_owner_cancel_own_booking' );
if( !function_exists('wpestate_owner_cancel_own_booking') ):
    function wpestate_owner_cancel_own_booking(){
        //scheck
        $current_user = wp_get_current_user();
        $userID         =   $current_user->ID;
...
if( !function_exists('wpestate_add_booking_invoice') ):
    function wpestate_add_booking_invoice(){
        ...
        $submission_curency_status = esc_html   ( get_post_meta($invoice_id, 'local_currency', true) );
        ...
        $reservation_plan_array = wpestate_get_booking_plans_room($listing_id);
        update_post_meta($listing_id, 'booking_plans_room', $reservation_plan_array);
        ...
        if($is_confirmed==1){
            ...
            update_post_meta($bookid, 'front_instants', $front_instants);

            $available_room = $property_emptys - $bookked_rooms;

            $bookked_rooms=$bookked_rooms+1;
            update_post_meta($listing_id, 'booked_rooms', floatval ($bookked_rooms));
            update_post_meta($bookid, 'booking_list_empty', floatval ( $property_emptys - $bookked_rooms));
            ...
            update_post_meta($bookid, 'booking_list_empty', $available_room);
        }
...
if( !function_exists('wpestate_booking_insert_invoice') ):
    function wpestate_booking_insert_invoice($billing_for,$type,$pack_id,$date,$user_id,$is_featured,$is_upgrade,$paypal_tax_id,$details,$price,$author_id=''){
        ...
        // 2023/2/29 デイユース開発 高橋（修正）---
        // $submission_curency_status = wpestate_curency_submission_pick();
        $submission_curency_status = esc_html   ( get_post_meta($property_id, 'local_currency', true) );
                        ...
                        //     </div>';
                            ...
                            foreach ($extra_options_array as $key=>$value){
                                if(isset($extra_pay_options[$value][0])){
                                    ...
                                    // 2023/6/11 デイユース開発 高橋（修正）
                                    // 以下を削除
                                    // <span class="inv_data inv_data_exp">'.$extra_option_value_show_single.'</span>
                                    print'
                                    <div class="invoice_row invoice_content">
                            ...
                            // 2023/6/11 デイユース開発 高橋（修正）---
                            $you_earn = floatval($booking_array ['youearned']) + floatval($booking_array ['taxes']);
                            $you_earn_show       =      wpestate_show_price_booking_for_invoice($you_earn,$wpestate_currency,$wpestate_where_currency,0,1);
                            ...
                            <div class="invoice_row invoice_totalx invoice_total_generate_invoice">
                                ...
                                <span class="inv_data" id="youearned" data-youearned="'.esc_attr($you_earn).'"><strong>'.$you_earn_show.'</strong></span>
...
if( !function_exists('wpestate_mess_front_end') ):
    function wpestate_mess_front_end(){
        ...
        $message_user .='<strong>'. esc_html__('From: ','wprentals').'</strong>'.$user_login.'<br>';
        $message_user .='<strong>'. esc_html__('Property: ','wprentals').'</strong>'.get_the_title($property_id).'<br>';
        $message_user .='<strong>'.esc_html__('Check-in: ','wprentals').'</strong>'.$booking_from_date.'<br>';
...
if( !function_exists('wpestate_ajax_show_booking_costs') ):
    function wpestate_ajax_show_booking_costs(){
        ...
        // print '</div>
        //         <div class="cost_value cost_planperprice" data_planperprice="'.$booking_array['inter_price'].'">'.$inter_price_show.'</div>
        //     </div>';

        print '<div class="show_cost_form" id="show_cost_form" >';
        ...
        print '
                <div class="cost_row" id="total_cost_row">
                    ...
                    <div class="cost_value" data_total_price="'.$booking_array['total_price'].'" >'.(($total_price_show) ? ($total_price_show) : 0).'</div>
                </div>
            </div>';
...
if (!function_exists('wpestate_post_review')):
    function wpestate_post_review(){
        ...
        *****/
        $post_lang = apply_filters( 'wpml_post_language_details', NULL, $listing_id );
        if( $post_lang['language_code'] == 'en' ) {
            $trans_id = apply_filters( 'wpml_object_id', $listing_id, 'post', false, 'ja' );
        } else {
            $trans_id = apply_filters( 'wpml_object_id', $listing_id, 'post', false, 'en' );
        };

        $data2 = array(
            'comment_post_ID' => $trans_id,
            'comment_author' => $user_login,
            'comment_author_email' => $user_email,
            'comment_author_url' => '',
            'comment_content' => $content,
        ...
        $comment_id2 =    wp_insert_comment($data2);
        add_comment_meta( $comment_id2, 'review_stars',$stars  );
        update_post_meta($trans_id,'review_by_'.$userID,'has');

        // wpestate_calculate_property_rating( $trans_id );
